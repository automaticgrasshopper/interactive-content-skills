...
- **触发与执行契约（最先执行）**：本 skill 被加载/命中（用户要“写个…故事 / 剧本 / 完整故事”），就等于已经要你交完整互动剧本。【加载 ≠ 完成】skill_load 只是把规范读进来，读完**必须立即按下面流程一次走完、一次交付**，只读不做视为失败：
  ...
  写完整剧本 —— ⚠ 写前又检查一次：我在写的是「剧本」还是「故事」？必须是分集剧本（每集可演可拍、台词原句贯穿、有分支选项与多结局），不是概述式故事/互动短篇
     ↓
  【验收回执·硬闸】交付前必须先填完第 6 节「交付前验收回执」（不是打勾，是把可核验的数字/复述填出来）——任一项不达标，禁止交付，先返工重写再重填，直到全部过
     ↓
  一次交付完整互动剧本（面向人：可读剧本；面向机：JSON 存文件）
  ```

  ⚠ **自检不得“打勾式”（治模型把自检当成形式、假跑）**：自检不是“我检查了✓”，而是把第 6 节验收回执里的**具体值真的算/数/复述出来**（路径几条、episodes 几个、新立几条规则、因果链一句话复述……）。数字和复述编不了，一写出来就自己抳到错（如“路径应 6 条、我写了 4 条”）。**禁止不算就答“通过”。**
...
- **入口①用户要“结构”**（分集结构 / 互动结构 / 故事骨架 / 怎么分集 / 剧情脉络 / 分支怎么走）：先给**可读的分集结构总览**——主线情绪脊（起承转合的情绪流）→ 共线段（每集一句话梳理）→ 选择点及各选项去向 → 分歧段 → 汇合点 → 结局及各自基调。结尾**主动问一句**：“整体结构就是这样。需要我把每一集的完整剧本（带台词、可直接开拍那种）逐集展开给你看吗？” 用户说要 → 再铺完整制片级剧本。（这层结构是真做完的 DAG、不是散文概述；情绪脊/共线/选择点/汇合/结局都是真实结构，只是用人话列给人看，仍不把 schema 字段名糊用户脸上。）
...
填完回执且全部达标，再过下面冷读五问与逐条清单（回执是“硬数据关”，五问与清单是“质感关”，两关都过才能交付）。

⚠ **冷读五问（每集写完先过这关，再过下面清单）**：写完一集，彻底扔掉你脑子里的大纲、设定、前情，装作一个**什么都不知道、只看得到这一集正文**的冷读观众，重读一遍并回答：①这里每个人是谁？②谁想要什么？③刚才发生了什么、为什么会这样？④那条规矩用大白话说是什么意思？⑤下一集你想看什么？**只要有一问答不出或要靠正文之外的信息才答得出，不是读者的问题，是剧本的问题，打回重写。**（观众就是零上下文的冷读者；作者读自己的东西永远是顺的，因为缺的环他自己脑补了——所以必须先卸掉上下文再读。）
