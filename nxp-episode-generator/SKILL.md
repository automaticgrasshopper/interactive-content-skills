...
- **正确写法（先戏后皮）**：先在脑子里把这一场戏写成**一段因果连着淌、读着顺的文学脚本**（动作—台词—反应一环扣一环，像旧 demo《听墓人》那种流动感），再把它**翻译/排版**成 △动作+人称+台词 的形式。流畅度必须和之前那份文学脚本**一模一样**，不允许因为换排版而回退。
...
- 【流畅度·排版不得伤流动】把任意一集的 △ 行与台词行**顺着读下来**，是不是像一段文学脚本一样因果连淌（动作勾台词、台词逼反应）？还是“一条动作、一句无关的话、又一条动作”的填表式跳跃？流畅度是否与旧 demo《听墓人》那种文学脚本一致、没因换排版而回退？（见 scene-flow.md）
