...
- **触发与执行契约（最先执行）**：本 skill 被加载/命中（用户要“写个…故事 / 剧本 / 完整故事”），就等于已经要你交完整互动剧本。【加载 ≠ 完成】skill_load 只是把规范读进来，读完**必须立即按下面流程一次走完、一次交付**，只读不做视为失败：
  ...
  写完整剧本 —— ⚠ 写前又检查一次：我在写的是「剧本」还是「故事」？必须是分集剧本（每集可演可拍、台词原句贯穿、有分支选项与多结局），不是概述式故事/互动短篇
     ↓
  【验收回执·硬数据关】交付前先填完第 6 节「交付前验收回执」（不是打勾，是把可核验的数字/复述填出来：路径数/结局数/字段/新立规则数/出场……）——结构类硬数据，本模块自己数得对；任一项不达标，返工重填，直到全部过
     ↓
  【冷读校验·可理解关】把剧本正文交给独立裁判 **nxp-episode-checker**（只读正文、不给它大纲/设定/前情），由它以零上下文观众身份答冷读五问——【为什么必须交给它、不能自己假装冷读】你脑子里装着纸面之外的全部前因后果，读自己永远是顺的（缺的环你自动脑补了），物理上当不了干净读者，所以这关必须换一个不知道上下文的第二方来判
     ↓
  checker 判“过” → 才算输出成功；判“不过” → 按它给的逐条理由与定位返工重写，再送检，直到过
     ↓
  一次交付完整互动剧本（面向人：可读剧本；面向机：JSON 存文件）
  ```

  ⚠ **自检不得“打勾式”（治模型把自检当成形式、假跑）**：自检不是“我检查了✓”，而是把第 6 节验收回执里的**具体值真的算/数/复述出来**（路径几条、episodes 几个、新立几条规则、因果链一句话复述……）。数字和复述编不了，一写出来就自己抓到错（如“路径应 6 条、我写了 4 条”）。**禁止不算就答“通过”。**

  **绝不得：先写一篇“概述式故事 / 带分支的短篇”应付、等用户追问“我要剧本”才开始写剧本。** 命中即全流程，内部必须一次把完整结构与剧本都做到位。

  ⚠ **但展示分流（禁的是“散文概述偷懒”，不是“先给结构”）**：“内部做到位”和“一次铺开多少给人看”是两回事。按用户要的东西分流（详见 2.5）：
  - 用户要的是**分集结构 / 互动结构 / 故事骨架 / 怎么分集 / 剧情脉络**：先给**可读的分集结构总览**（主线情绪脊→共线段→选择点→分歧段→汇合→结局），结尾主动问一句“需要把每一集的完整剧本（带台词、可直接开拍）展开给你看吗？”——这不是偷懒，结构是真做完的、剧本随时能铺。
  - 用户要的是**剧本 / 完整故事 / 把剧本写出来 / 要台词**：直接上**完整制片级剧本**，不问、不摘要、一次到位。
  - **判断模糊时默认走“完整剧本”**（宁多给、不偷懒）。无论哪条，内部都已把完整结构与剧本做完，区别只在展示多少。
...
旁路质检：nxp-episode-checker（只读本模块产出的剧本正文、只出“过/不过”判决，**不改产物、不动 schema/字段、不碰上下游接口参数**）——它是交付前的可理解性裁判，拆不拆它，本模块对上下游的输入/输出字段与口径一字不变。
...
- 人物静态设定（外貌 / 服装 / 音色 / 身份锚点）作为**结构化字段**不复制不重发——下游机器从资产层直接取；本模块只产出逐集变化的动态人物状态（目标 / 情绪 / 关系 / 信息）。**但⚠ 这是接口纪律不是叙事豁免：观众不读资产层，新角色的戏内出场引入（谁/从哪冒出来/为什么在这）必须在 plot 里写，见 3.2 新角色出场三要素。**镜头 / 景别 / 机位 / 光线等执行层内容一律不写。
...
⚠ **展示分流（按用户要什么分流，两种入口不同开场；内部一律把完整结构+剧本做完，区别只在展示多少）：**

- **入口①用户要“结构”**（分集结构 / 互动结构 / 故事骨架 / 怎么分集 / 剧情脉络 / 分支怎么走）：先给**可读的分集结构总览**——主线情绪脊（起承转合的情绪流）→ 共线段（每集一句话概括）→ 选择点及各选项去向 → 分歧段 → 汇合点 → 结局及各自基调。结尾**主动问一句**：“整体结构就是这样。需要我把每一集的完整剧本（带台词、可直接开拍那种）逐集展开给你看吗？” 用户说要 → 再铺完整制片级剧本。（这层结构是真做完的 DAG、不是散文概述；情绪脊/共线/选择点/汇合/结局都是真实结构，只是用人话列给人看，仍不把 schema 字段名糊用户脸上。）
- **入口②用户要“剧本”**（写个…故事 / 给我看剧本 / 把剧本写出来 / 要台词 / 完整输出）：**直接上完整可读剧本**（分集正文 + 台词原句 + 选项分支 + 多结局），**不问、不摘要、一次到位**，不得以“已写进文件”或“先给你看结构”为由只给摘要或反问拖延。
- **判断模糊（如只说“搞个××”）默认走入口②**（宁多给不偷懒）。

⚠ **剧本成品必须优先、完整展示，绝不压缩（适用于入口②，以及入口①用户说要展开之后；展示形态见 §铁则「输出双态」：给人看的是可读剧本，不是 JSON）：**

- 展开完整剧本时，最高优先级是把**可读剧本完整展示**（分集正文 + 台词原句 + 选项分支 + 多结局），不得以"已写进文件"为由只给摘要。
...
制片级剧本格式（plot 的写法——**格式只是皮，内核是一部流畅的文学脚本**；写前必读 `references/scene-flow.md`，合格成品长什么样见 `references/gold-sample-episode.md` 金标样例）：

⚠⚠ **最要命的一条（治“换了格式后变得碎、不流畅”）：制片级格式只是一层呼应“皮”，不是写作方式。** —— 把 `△动作提示 + 人称 + 台词` 这套排版暴露给外行（制片/投资方），只是为了让他们觉得“专业、像那么回事”；行内导演本来就能读文学脚本、也需要留白空间。**内核从头到尾都是那部流畅的文学脚本，只是换了个形式写。**

- **正确写法（先戏后皮）**：先在脑子里把这一场戏写成**一段因果连着淌、读着顺的文学脚本**（动作—台词—反应一环扣一环），再把它**翻译/排版**成 △动作+人称+台词 的形式。流畅度的唯一标杆是**真人写的自然剧本语料**（见 scene-flow.md 2.5），不是本管线自己生成过的任何剧本；不允许因为换排版而回退。
- **禁填表式碎场（这次回旋的真凶）**：绝不得对着 `△一句—台词一句—△一句` 的空格子**填空**（填一个△、一两句台词就切场）——那会把连贯的一场戏切成互不相干的碎块、因果流断掉。判失败。
- **△ 不是“一句孤立动作条”**：一个 △ 可以承载连贯的动作流与必要环境（“她没接碗，低头嗅了嗅，指尖蘸一点抵在舌尖，眉头先松后紧”），让场子淌起来；不是一行只能写一个小动作。
- **台词行行分离只是排版**：台词按 **角色名：台词** 逐句列出，与 △ 动作分行（方便下游看）；但分行不等于切断——上一句台词要能勾出下一个动作、这个动作要能逼出下一句台词，因果链不能断。
  ```
  【场1 · 地点 · 日/夜 · 内/外】
  出场：角色A、角色B
  ...
  角色B：台词原句
  △ （接住上一句、勾出下一句的动作）
  （同一集可有多个场次：【场2·…】，但场与场之间要有推力承接，不是硬切）
  ```
  叙述只写能拍的客观动作/环境，不写“他心里想…”这类不可拍的内心。
- 写 plot = 写剧本终稿。不是写“这集大致讲什么”的概要，而是把这一场戏完整、流畅地写出（台词逐句、动作分行，但因果连淌），该长就长，不为省 token 压成概述。
- 信息从戏里自然长出来，禁硬灌：背景/身世/世界规则要由当下的动作与台词带出来，**禁用“路人画外音/村妇嘴替”成段倒背景**（那是贴上去的信息、不是从戏里长出来的，最典型的不流畅）。
- 第一原则：本生成框架难以靠复杂调度完成戏剧（多动作 / 走位是下游视频生成的漂移源），**戏剧张力优先由台词承载，调度保持简单**；动作做台词的标点与因果链环节（合上文件、递烟、停顿），不扛大调度。台词越充分，成片故事越完整。
...
- **前提一遍看懂（与反套路同时成立，两头都不许倒）**：故事前提与主角身份（如“现代医生穿到古代”）属于**功能信息**，第一集必须让观众**一遍看懂**——反套路不等于把前提藏没（砍到观众不知道主角是谁、为什么会这些，同样判失败）。交代手段必须是戏内的：说漏嘴的现代词（“我是医生——我是大夫！”）、冲突中被迫自报、他人质问带出身份落差；禁照镜自述、禁旁白倒设定。人物关系（谁是谁的谁）同理，第一集内在对话/冲突里自然带清。
...
⚠ **新角色出场三要素（禁凭空出现、禁凭空开口）**：任何角色第一次出场，必须在戏内给齐三样——①**出场动作**：△ 里写清他从哪冒出来、什么样子（真语料范式：“被一个人喊住了……抬头一看，喊他的是王瘸子”）；②**身份锚**：观众一遍知道他是谁/什么身份（靠称呼、行当行头、他人反应带出，不靠旁白）；③**来意/与主角的关系**：他为什么在这、凭什么和主角搭上话（“我的鸡得病了，你快去帮我看看”）。第一句台词之前至少①必须到位——**绝不允许一个没有任何交代的名字直接开口说话**。（注：这与“静态设定不重发”不矛盾：那是结构化字段的接口纪律，这是写给观众看的叙事义务，两回事。）

⚠ **世界规则预算·先立后用（治“前因后果乱”的病根，规则怪谈/悬疑/玄幻题材高发）**：

- **规则只能用已立的**：本集可用的世界规则，以上游大纲的系统规则 + 前面集数已立过的为限。**任何后果只能兑现此前已立过的规则；禁止为了推下一拍当场现编一条新规则来解释当下。**
- **新规则预算：每集最多新立 1 条**，且立的时候必须大白话讲透、被在场角色接住（追问/确认），然后**先用一次再让它决定大后果**（立→验→兑现）。一场戏里连立三五条规则（“认一句就上岸/给名字就跟谁/叫水鬼就找替身”一个接一个），观众刚记住一条下一条又改写利害——那不是讲故事，是边玩边改游戏规则，判失败重写。
- **规则不漂移**：同一条规则前后口径一致，不随剧情需要变形。
- 题材透镜的“反差物/双义钩子”**不等于新规则**：转折优先用已立规则的新用法/新代价来做，而不是发明新规则——新规则是最廉价也最伤因果的转折。

⚠ **关键因果拍必须上台面（禁塞进 △ 概述）**：验真测试、问答、交易、决定这类承因果的拍，必须用**原句台词 + 可见动作**演出来；禁把它们压进 △ 叙述（✗“沈砚问起童年河边的错事。水里答‘是你脚滑’”——问题没演、真相靠旁白倒灌，观众只能脑补因果）。△ 里也禁写不可拍的背景真相（“这句话连母亲都不知道”）。

⚠ **动机可指认（禁无铺垫变脸）**：角色每次立场/行为突转（帮手突然抢东西、盟友互挡），必须有前文可指认的动机铺垫；双义钩子 ≠ 人人随时变脸。

⚠ **台词呼吸感（治“句句金句”病，与高密度不矛盾）**：高密度指“信息和戏由台词承载”，**不指每句都掷地有声**。真人对话大量是搭话、确认、迟疑、反应（“是你叫我，有什么事？”“怎么没用，你是大学生嘛”），承重句是少数。一场戏里**承重句（传设定/转折/命脉）不超过三分之一**，其余是接得住的普通人话来回；连续三句以上句句设定金句（“这个潭不吃蚓蚓吃名字”式一个接一个），判“设定发布会”，重写。流畅度的标杆是**真人写的自然剧本语料**（chinese-dialogue-craft 取样源），不是本管线自己生成过的任何剧本。
...
- **填表式碎场（换了制片格式后流畅度回退的真凶）**：把制片格式当写作方式，对着 `△一句—台词一句—△一句` 空格子填空，填一个△+一两句台词就切场；把连贯流动的一场戏切成互不相干的碎块、因果链断掉。流畅度必须与之前文学脚本一致，不得因换排版而回退（见 scene-flow.md）。
- **信息硬灌**：用路人画外音/村妇嘴替成段倒背景、身世、世界规则；信息是贴上去的而非从戏里长出来的。场与场硬切拼贴、无推力承接。
- **角色凭空出现**：新角色无出场动作、无身份锚、无来意，一个没任何交代的名字直接开口与主角对话（把“静态设定不重发”接口纪律误读成叙事豁免）。
- **句句金句（设定发布会）**：每句单看都短、都不翻译腔，但句句掷地有声、句句传设定，无搭话/确认/迟疑/反应这些普通来回——单句像人话、合起来不像人对话，呼吸感为零。
- **当场现编规则（前因后果乱的直接来源，规则怪谈/玄幻高发）**：每需要推下一拍就当场发明一条新超自然规则，一场戏连立三五条、每条都改写利害关系；后果兑现的不是已立规则而是刚发明的规则。
- **关键因果拍被塞进 △ 概述**：验真/问答/交易这类承因果的拍不演、靠旁白解释，真相靠叙述倒灌（“这句话连母亲都不知道”），观众只能脑补因果链。
- **无铺垫变脸**：角色立场突转无前文动机铺垫（帮手突然抢夺、盟友瞬间互为敌手），把双义钩子执行成人人随时变脸。
...
⚠⚠ **交付前验收回执（硬闸：不填不得交，填了不达标也不得交）**

交付前，必须**逐项把下面这张回执的实际值填出来**（在内部算/数/复述，不给用户看）。**这不是打勾，是填空：每一项都要写出可核验的具体值或一句复述，写“已检查/OK/通过”无效。** 任一项填不出、或填出来不达标——立即停下返工，不得交付：

```
【上游】大纲来源=？   engine=？   episodeCount=？   endingCount=？   情绪脊=（齐/缺）
【结构】episodes 实际数=？（须=episodeCount）   结局 final+route 叶子数=？（须=endingCount）
【路径】从 ep_001 到各结局的完整路径共=？条（自己沿 edges 数一遍，不凭印象）   max_path_length=？
【规则账本】本集（或各集）新立超自然/世界规则=？条（须≤1/集）   每个后果兑现的是哪条已立规则=？
【因果链】抽 1–2 集，用一句话复述它的因果（谁因为什么做了什么、导致什么）=？（复述不出即乱）
【字段】抽 1 集，dramatic_core/emotional_owner/power_shift/information_gap/dialogue_intent/character_beats 是否逐项有值=？
【字数】最短的一集 plot 字数=？（须≥350，≤2集放宽≥250）
【出场】本次新出场角色共=？人，逐个确认首句台词前有出场动作=？（有凭空开口的列出来）
```

填完回执且全部达标（硬数据关），再把正文交冷读校验（可理解关），最后走下面逐条清单。

⚠⚠ **冷读校验（可理解关）= 交给独立裁判 nxp-episode-checker，不自己假装**：

“观众看不看得懂”这一类检查，**本模块自己验不了**——你脑子里装着纸面之外的全部前因后果（柳婆婆为什么变脸、“是你脚滑”为什么错你都知道），读自己写的东西永远是顺的，缺的环你自动脑补了。这是知识诅咒，**同一个脑子在同一口气里物理上卸不掉自己已知的东西、装不成不知道。**

所以交付前，把剧本正文（只给正文，**不给大纲/情绪脊/设定/前情**）交给 **nxp-episode-checker**，由它以零上下文观众身份逐集答冷读五问（①每个人是谁？②谁想要什么？③刚才发生了什么、为什么？④那条规矩大白话是什么意思？⑤下一集想看什么？），给出“过 / 不过 + 逐条理由与定位”。**判“过”才算输出成功；判“不过”就按它的理由返工重写、再送检，直到过。** （单用无法调 checker 时：至少把“卸上下文再读”当成兵线尽力自验，但要知道这比换脑子弱。）

输出前检查：

- 【输出纪律】是否没有向用户陈述结构判断/自检过程/台词工艺这些内部工序？
- 【展示分流】分清用户要的是“结构”还是“剧本”了吗？要结构→先给可读结构总览并结尾主动问“要不要展开每集完整剧本”？要剧本（或模糊）→直接上完整剧本、不问不摘要？（无论哪条，内部完整结构+剧本都已做完）
- 【剧本优先】用户要剧本（或入口①已同意展开）时，是否把剧本**完整展示**了（非摘要、非“已写进文件”打发、非“先给你看结构”拖延）？plot 是否达字数下限、台词为原句、未为省 token 压缩？
...
- 【流畅度·排版不得伤流动】把任意一集的 △ 行与台词行**顺着读下来**，是不是像一段文学脚本一样因果连淌（动作勾台词、台词逼反应）？还是“一条动作、一句无关的话、又一条动作”的填表式跳跃？流畅度是否对得上真人自然剧本语料的流动感、没因换排版而回退？（见 scene-flow.md 2.5）
- 【信息自然流出】背景/身世/世界规则是不是从当下的动作与台词里自然带出？有没有用路人画外音/嘴替成段硬灌背景？场与场之间有推力承接还是硬切拼贴？
- 【新角色出场】每个首次出场的角色，出场三要素（出场动作/身份锚/来意）齐了吗？有没有哪个名字是凭空冒出来直接开口的？
- 【呼吸感】承重句（传设定/转折/命脉）是否不超一场戏的三分之一？有没有连续三句以上句句金句的“设定发布会”？搭话/确认/迟疑/反应这些普通来回在不在？流畅度对标的是真人语料而非自己的旧产出吗？
- 【规则预算·因果链】本集新立的超自然/世界规则是否 ≤1 条？每个后果兑现的都是**此前已立过**的规则、而非当场现编的？同一规则前后口径一致无漂移？把本集因果链用一句话复述一遍（谁因为什么做了什么、导致什么）——复述不出来就是乱的。
- 【因果拍上台面】验真/问答/交易/决定这类承因果的拍，是用原句台词+可见动作演出来的，还是被塞进 △ 概述/靠旁白倒真相了？角色立场突转有前文可指认的动机铺垫吗？
...
- 【开篇集】第一集是否让**金手指在一个具体事件里第一次发挥作用**、结尾甩出具体迫近钩子？没有花整集纯铺氛围/立人设、也没走训练集均值穿越开场（摸脸→照镜→自述身份）吧？**前提与主角身份是否一遍看懂**（反套路没把前提砍没，交代靠戏内手段而非照镜自述/旁白）？人物关系第一集内带清了吗？
