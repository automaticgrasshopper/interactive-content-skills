...
开始冷读时向用户播报`正在对照细节`。全部内容检查通过、开始组装正式正文时播报`正在完成终稿`；开始把通过检查的正文写入当前分集结果时播报`正在合并剧本`。阶段失败不得提前播报后一阶段。

按`references/dialogue-and-quality.md`分别检查首次出场、空间与动作连续、非台词场面化、交流段功能、对白口语复写、事实忠实、停止边界和资产一致性。组装现有节点草稿合同，其中`分集剧本.完整剧本`必须逐字等于`enhanced-screenplay.md`；随后运行：
