...
class ArchitectureTests(unittest.TestCase):
    ...
    def test_spatial_continuity_requires_causal_blocking_not_action_filler(self):
        ...
        self.assertIn("维持一条可追踪的场面调度链", enhancer)
        self.assertIn("不要在每句台词间机械插入", enhancer)
        self.assertIn("逐人核对最终位置与关键物件归属", enhancer)
        self.assertIn("正文已经写出每个人到达同一位置", enhancer)
        cold_read = (ROOT / "references" / "node-screenwriting.md").read_text(encoding="utf-8")
        self.assertIn("逐场做一次空间接力冷读", cold_read)
        self.assertIn("身份上的`保管`不能代替现场来源", cold_read)
    ...
    def test_manifest_inherits_008(self):
        manifest = json.loads((ROOT / "reference-manifest.json").read_text(encoding="utf-8"))
        self.assertEqual(manifest["based_on"], "episode-screenwriter-biz/0.08")
        self.assertEqual(manifest["skill_version"], "episode-screenwriter-biz/0.09")
