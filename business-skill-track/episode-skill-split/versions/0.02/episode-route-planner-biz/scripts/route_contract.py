...
SKILL_VERSION = "episode-route-planner-biz/0.02"
ROOT_FIELDS = {
...
DRAMATIC_PRESSURE_FIELDS = {"fact_trigger", "felt_meaning"}
DRAMATIC_EFFECT_FIELDS = {"resulting_action", "human_change", "later_effect"}
...
def validate(route: Any, *, require_accepted: bool) -> list[str]:
    ...
    if route["skill_version"] != SKILL_VERSION:
        issues.append("路线Skill版本错误")
    ...
    for index, node in enumerate(nodes):
        ...
        if exact_keys(material, MATERIAL_FIELDS, f"{node_id}/route_material", issues):
            ...
            entry_state = material.get("entry_state")
            pressure = entry_state.get("dramatic_pressure") if isinstance(entry_state, dict) else None
            state_changes = material.get("state_changes")
            effect = state_changes.get("dramatic_effect") if isinstance(state_changes, dict) else None
            if (pressure is None) != (effect is None):
                issues.append(f"{node_id}/双因果压力与结果必须同时存在")
            if pressure is not None and exact_keys(pressure, DRAMATIC_PRESSURE_FIELDS, f"{node_id}/dramatic_pressure", issues):
                for field in DRAMATIC_PRESSURE_FIELDS:
                    if not non_placeholder(pressure[field], 6):
                        issues.append(f"{node_id}/dramatic_pressure/{field}为空或占位")
            if effect is not None and exact_keys(effect, DRAMATIC_EFFECT_FIELDS, f"{node_id}/dramatic_effect", issues):
                for field in DRAMATIC_EFFECT_FIELDS:
                    if not non_placeholder(effect[field], 6):
                        issues.append(f"{node_id}/dramatic_effect/{field}为空或占位")
    ...
    if ending_types != ENDING_TYPES:
        issues.append(f"四类结局不完整：{sorted(ENDING_TYPES - ending_types)}")
