...
def validate(candidate: Any, evidence: Any) -> list[str]:
    ...
    movement_fields = {
        "movement_id", "segment_ids", "source_proof", "fact_trigger", "felt_meaning",
        "resulting_action", "human_change", "later_effect", "payoff_node_ids", "candidate_fissures",
    }
    ...
    for index, movement in enumerate(movements):
        ...
        for field in ("fact_trigger", "felt_meaning", "resulting_action", "human_change", "later_effect"):
            if len(str(movement[field] or "").strip()) < 6:
                errors.append(f"情绪运动缺少{field}：{movement_id}")
        source_node_ids = {str(segment_by_id[item]["route_node_id"]) for item in segment_ids}
        source_material = json.dumps(
            [by_id[node_id].get("route_material", {}) for node_id in source_node_ids if node_id in by_id],
            ensure_ascii=False,
        )
        for field in ("fact_trigger", "felt_meaning", "resulting_action", "human_change", "later_effect"):
            value = str(movement[field] or "").strip()
            if value and value not in source_material:
                errors.append(f"双因果运动未进入正式节点材料：{movement_id}/{field}")
        payoff_node_ids = movement["payoff_node_ids"]
        source_is_ending = bool(source_node_ids) and all(by_id[node_id].get("是否结局") is True for node_id in source_node_ids)
        if not isinstance(payoff_node_ids, list) or any(
            node_id not in by_id
            or node_id in source_node_ids
            or not any(reachable(successors, source, node_id) for source in source_node_ids)
            for node_id in payoff_node_ids
        ):
            errors.append(f"双因果运动后续消费节点非法：{movement_id}")
        elif not source_is_ending and not payoff_node_ids:
            errors.append(f"非结局双因果运动没有后续消费节点：{movement_id}")
        else:
            later_effect = str(movement["later_effect"] or "").strip()
            if payoff_node_ids and not any(
                later_effect in json.dumps(by_id[node_id].get("route_material", {}), ensure_ascii=False)
                for node_id in payoff_node_ids
            ):
                errors.append(f"人物或关系变化没有实际影响后续行动：{movement_id}")
    ...
    action_fields = {
        ...
        "core_goal_status", "consumed_at_node_ids", "emotional_basis", "human_cost",
    }
    branch_nodes = {
    ...
    for index, fissure in enumerate(fissures):
        ...
        for action_index, action in enumerate(actions):
            ...
            emotional_basis = str(action["emotional_basis"] or "").strip()
            human_cost = str(action["human_cost"] or "").strip()
            if len(emotional_basis) < 6 or len(human_cost) < 6 or human_cost in human_costs:
                errors.append(f"选项缺少人物动机或互异的人物代价：{source}->{target}")
    ...
    for index, choice in enumerate(treatment_choices):
        ...
        for option_index, option in enumerate(options):
            ...
            emotional_motive = str(option["emotional_motive"] or "").strip()
            human_cost = str(option["human_cost"] or "").strip()
            ...
            if len(emotional_motive) < 6 or len(human_cost) < 6 or human_cost in human_costs:
                errors.append(f"支线事实选项缺少人物动机或互异人物代价：{choice_id}/{option_id}")
    ...
    spine_ids: list[str] = []
    turns = 0
    movements_seen_in_spine: set[str] = set()
    for index, item in enumerate(spine):
        if not exact_fields(item, {"node_id", "valence", "arousal", "dominance", "turn", "turn_reason", "source_quote", "causal_movement_ids"}, f"情绪脊/{index}", errors):
            continue
        ...
        causal_ids = item["causal_movement_ids"]
        if not isinstance(causal_ids, list) or any(value not in movement_by_id for value in causal_ids):
            errors.append(f"情绪脊双因果引用非法：{node_id}")
        else:
            movements_seen_in_spine.update(causal_ids)
    ...
    if movements_seen_in_spine != set(movement_by_id):
        errors.append("情绪脊没有检查全部双因果运动")
