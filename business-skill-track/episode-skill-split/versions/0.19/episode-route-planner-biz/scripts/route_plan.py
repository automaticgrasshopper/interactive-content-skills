"""0.19: frozen mainline, scoped branch repair, graph inspection and exact story projection."""
import argparse
import hashlib
import json
import os
...
def policy(root):
    ...
    exemptions = {}
    allowed_rules = {'small', 'crossing', 'depth', 'ending_types', 'ending_distribution', 'immediate_result'}
    ...
    if counts['choice_node_count'] == 0:
        ...
        for key in ('small', 'crossing', 'depth', 'ending_distribution'):
...
def freeze(root):
    if (root/'mainline-lock.json').exists():
        return frozen(root)
    value = policy(root)
    story_parts(read(root/'mainline.json'))
    lock = {'contract_version': 'route-019.mainline-lock.v1', 'created_at': stamp(), 'policy': value,
            'files': {name: sha(root/name) for name in ('user-request.md', 'brief.json', 'mainline.json')}}
...
def submit(root, submission):
    ...
    state = {'contract_version': 'route-019.topology.v1', 'revision': old['revision']+1 if old else 1,
...
def materialize(root, state):
    ...
    route = {'contract_version': handoff.CONTRACT_VERSION, 'capability_id': handoff.CAPABILITY_ID,
             'project_id': brief['project_id'], 'route_id': brief.get('route_id', 'route-019'),
...
def accept(root, output=None):
    ...
    receipt = {'contract_version': 'route-019.planning.v1', 'status': 'PLANNING_ACCEPTED',
...
def status(root):
    for name in ('user-request.md', 'brief.json', 'mainline.json'):
