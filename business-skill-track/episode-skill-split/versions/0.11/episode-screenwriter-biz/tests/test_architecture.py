...
def route(node_id: str = "episode-001", predecessors: list[str] | None = None) -> dict:
    return {
        "project_id": "p",
        "route_id": "r",
        "route_version": "1",
        ...
        "nodes": [{
            ...
            "前置节点编号列表": list(predecessors or []),
        }],
    }


def context(node_id: str = "episode-001", predecessors: list[str] | None = None) -> dict:
    return {
        ...
        "direct_predecessor_endings": [
            {"node_id": item, "ending_excerpt": "上一节点的真实正文结尾。"}
...
class ArchitectureTests(unittest.TestCase):
    ...
    def test_viewer_mode_uses_topology_not_episode_id(self):
        entry_packet = build_packet(route("episode-777"), context("episode-777"), "episode-777")
        entry_payload = json.loads(entry_packet.split("\n", 1)[1])
        self.assertEqual(entry_payload["viewer_mode"], "entry")

        predecessors = ["episode-000"]
        continuation_packet = build_packet(
            route("episode-001", predecessors),
            context("episode-001", predecessors),
            "episode-001",
        )
        continuation_payload = json.loads(continuation_packet.split("\n", 1)[1])
        self.assertEqual(continuation_payload["viewer_mode"], "continuation")

    def test_viewer_mode_is_internal_and_does_not_expand_business_contract(self):
        self.assertEqual(CONTEXT_VERSION, "nextplay.episode-screenwriting-context.v1")
        self.assertNotIn("viewer_mode", CONTEXT_FIELDS)
        self.assertNotIn("audience_orientation", REQUIRED_CHECKS)
    ...
    def test_entry_orientation_hitchhikes_on_existing_action(self):
        compact = (ROOT / "references" / "compact-action-draft.md").read_text(encoding="utf-8")
        enhancer = (ROOT / "references" / "full-scene-enhancer.md").read_text(encoding="utf-8")
        review = (ROOT / "references" / "node-screenwriting.md").read_text(encoding="utf-8")
        self.assertIn("不另写背景说明段", compact)
        self.assertIn("安放到原有因果链的一拍中", compact)
        self.assertIn("附着在底稿已有的执行、阻力、反馈或代价上", enhancer)
        self.assertIn("第一轮真正改变局面的行动", review)
        combined = compact + enhancer + review
        self.assertNotIn("三至五句", combined)
        self.assertNotIn("主要动作前", combined)

    def test_first_use_keeps_mystery_and_avoids_repetition(self):
        enhancer = (ROOT / "references" / "full-scene-enhancer.md").read_text(encoding="utf-8")
        self.assertIn("第一次真正允许、阻挡或惩罚行动", enhancer)
        self.assertIn("可以保留来源和真相为悬念", enhancer)
        self.assertIn("不重复首集已经成立的处境和规则", enhancer)
    ...
    def test_manifest_inherits_010_as_active(self):
    ...
    def test_active_pointer_selects_011_without_changing_route_skill(self):
        ...
        self.assertEqual(ownership["screenplay_skill"], "episode-screenwriter-biz/0.11")
