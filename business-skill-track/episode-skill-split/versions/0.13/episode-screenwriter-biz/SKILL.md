...
description: "当且仅当已有由流程图 Skill 正式保存并验收的互动路线，用户要求为其中一个指定节点生成、预览、修改、检查或验收正式分集剧情、场景、对白和完整剧本，或在正式剧本完成后的继续对话中形成改名、正文修改或新的完整剧本时使用本 Skill。一次只处理一个node_id；路线不存在、未验收、版本或哈希失效时停止，不自行规划、重建或改写拓扑。"
---
...
正式路线是只读依赖。一次只接收一个`node_id`，并同时绑定`project_id`、`route_id`、`route_version`、`route_output_hash`和`node_route_material_hash`。重要人物行动节点的路线材料必须包含已由上游冻结的双因果：事实触发、人物对事实的理解、由此发生的行动、人或关系变化及其后续行为影响；纯功能过桥节点不强制制造情感变化。此外必须收到按`references/business-interface.md`绑定的`单节点写作上下文`，其中包含当前允许人物的公开身份与当前相关性、允许场景和道具的公开说明，以及全部直接前情正文结尾。
...
只读取写作包与`references/compact-action-draft.md`，写`compact-draft.md`。本阶段只决定这场戏有哪些真正改变局面的戏剧拍，以及它们的双因果顺序和停止位置；人物行动必须同时承接外部事实与人物赋予该事实的意义，并让冻结的人或关系变化成为可见结果。入口节点同时决定观众理解所需事实由哪个现有戏剧拍承载，但不另造背景说明拍，也不负责正式人物介绍、空间调度、成品对白或语言风格。
...
底稿通过后生成唯一 Enhancer 输入。Enhancer只读取该输入，依据`references/full-scene-enhancer.md`负责整场表达，并只在写对白时使用`references/dialogue-and-quality.md`；其中真人短剧参照按当前关系与交流任务最多选择一种，举证升级和关系反噬只在底稿已有相应戏剧拍时启用。它是唯一正式剧本写作者，不保留底稿原句的义务，只能表达底稿已有戏剧拍，不增加证明轮次、解释轮次或装饰性调度。
