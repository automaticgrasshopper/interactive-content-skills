...
这些是内部图形检查与创作目标；检查失败不阻断Agent业务流程，按[恢复规则](acceptance.md)局部处理或直接交接，不伪造PASS。
...
先按故事整理解决用户目标与实际体量的冲突，再使用冻结分类。用户要求短时长却又要求完整展开多阶段故事，不能仅填写短时长就豁免交织、深度与持续展开；须先得到对时长或故事范围的明确取舍。无时长要求且实际故事有持续展开空间时，保留非短故事复杂图形，不能默认缩短以套用简化门禁。机器只执行已确定的分类，不证明作者的体量判断合理。
