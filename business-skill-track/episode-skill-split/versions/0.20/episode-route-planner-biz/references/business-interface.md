...
- 剧集`单集梗概`取已完成故事切片的完整叙事正文，不带文档标题、Markdown章节编号或主线/支线编排说明；标题单独写入`分集标题`，不得在保存时另缩成结论摘要。主线、拓扑新增剧集和共享剧集使用同一材料标准，选择卡材料仅含问题，不承载独立剧情；状态填有依据的事实，资产使用实际出现的正式名称，不能编造状态。
