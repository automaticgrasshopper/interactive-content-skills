...
HARD_COUNT_FIELDS = {"episode_count", "choice_node_count", "ending_count"}
...
def route_counts(candidate: Any) -> dict[str, int]:
    nodes = candidate.get("nodes") if isinstance(candidate, dict) and isinstance(candidate.get("nodes"), list) else []
    def is_choice_node(node: Any) -> bool:
        return (
            isinstance(node, dict)
            and isinstance(node.get("互动节点"), dict)
            and node["互动节点"].get("是否为分支节点") is True
        )

    return {
        "episode_count": sum(isinstance(node, dict) and not is_choice_node(node) for node in nodes),
        "choice_node_count": sum(is_choice_node(node) for node in nodes),
        "ending_count": sum(
            isinstance(node, dict) and node.get("是否结局") is True for node in nodes
        ),
    }


def validate_user_counts(candidate: Any, user_intent: Any, errors: list[str]) -> None:
    hard_counts = user_intent.get("hard_counts") if isinstance(user_intent, dict) else None
    if not isinstance(hard_counts, dict) or set(hard_counts) != HARD_COUNT_FIELDS:
        errors.append("用户意图锁必须完整列出三项hard_counts")
        return
    actual = route_counts(candidate)
    for field, expected in hard_counts.items():
        if expected is None:
            continue
...
def validate_ending_policy(candidate: Any, user_intent: Any, errors: list[str]) -> None:
    hard_counts = user_intent.get("hard_counts") if isinstance(user_intent, dict) else None
    if not isinstance(hard_counts, dict):
        return
    nodes = candidate.get("nodes") if isinstance(candidate, dict) else None
    if not isinstance(nodes, list):
        return
    ending_types = {
        node.get("ending_type")
        for node in nodes
        if isinstance(node, dict) and node.get("是否结局") is True
    }
    ending_count = hard_counts.get("ending_count")
    if ending_count is None:
        missing = ENDING_TYPES - ending_types
        if missing:
            errors.append(f"默认四类结局不完整：{sorted(missing)}")
...
def build_from_root(cache_root: Path) -> dict[str, Any]:
    user_intent = load_json(cache_root / "user-intent-lock.json")
    if user_intent.get("shape_mode") == "exact":
        selection_errors = verify_topology_selection(cache_root)
        if selection_errors:
            raise ValueError("用户锁定拓扑未通过：" + "；".join(selection_errors))
        candidate = load_json(cache_root / "route-candidate.json")
        errors = validate_route(candidate, require_accepted=False)
        validate_user_counts(candidate, user_intent, errors)
        validate_ending_policy(candidate, user_intent, errors)
        lock = user_intent.get("locked_shape")
        errors.extend(shape_lock_issues(lock))
        if not errors and not route_satisfies_exact_lock(candidate, lock):
            errors.append("正式候选没有精确实现用户锁定形状")
        if errors:
            raise ValueError("；".join(errors))
        files = {name: file_sha256(cache_root / name) for name in EXACT_REQUIRED_FILES}
        evidence = {"user_intent": user_intent, "route_candidate": candidate}
        return {
            "contract_version": RECEIPT_VERSION,
            "status": "PASS",
            "project_id": candidate["project_id"],
    ...
    validate_user_counts(candidate, user_intent, count_errors)
    validate_ending_policy(candidate, user_intent, count_errors)
    if count_errors:
...
def verify_root(cache_root: Path) -> list[str]:
    try:
        ...
        if user_intent.get("shape_mode") == "exact":
            selection_errors = verify_topology_selection(cache_root)
            if selection_errors:
                return ["用户锁定拓扑未通过：" + "；".join(selection_errors)]
            candidate = load_json(cache_root / "route-candidate.json")
            errors = validate_route(candidate, require_accepted=False)
            validate_user_counts(candidate, user_intent, errors)
            validate_ending_policy(candidate, user_intent, errors)
            ...
            errors.extend(verify_receipt(candidate, receipt))
            evidence = {"user_intent": user_intent, "route_candidate": candidate}
            if receipt.get("planning_evidence_hash") != digest(evidence):
                errors.append("用户锁定路线材料在验收后发生变化")
            current_files = {name: file_sha256(cache_root / name) for name in EXACT_REQUIRED_FILES}
            if receipt.get("files") != current_files:
                errors.append("用户锁定路线验收绑定的文件已失效")
            return list(dict.fromkeys(errors))
        selection_errors = verify_topology_selection(cache_root)
        if selection_errors:
