...
EXACT_FILE_STEPS = (
    ...
    ("decision-fissure-audit.json", "BUILD_DECISION_FISSURES"),
    ("story-treatment.json", "BUILD_STORY_TREATMENT"),
    ("topology-draft-1.json", "WRITE_LOCKED_TOPOLOGY_ONCE"),
    ("topology-selection.json", "SEAL_LOCKED_TOPOLOGY"),
    ("route-candidate.json", "MATERIALIZE_LOCKED_TOPOLOGY"),
    ("planning-acceptance.json", "ACCEPT_PLANNING"),
)

FILE_STEPS = OPEN_FILE_STEPS
...
def status(cache_root: Path) -> dict[str, Any]:
    ...
    steps = OPEN_FILE_STEPS
    intent_path = root / "user-intent-lock.json"
    if intent_path.is_file():
        ...
        if intent.get("shape_mode") == "exact":
            steps = EXACT_FILE_STEPS
    first_missing = next((index for index, (name, _) in enumerate(steps) if not (root / name).is_file()), None)
    if first_missing is None:
        return {"contract_version": VERSION, "status": "COMPLETE", "phase": "BRANCH", "next_actions": []}
    future = [name for name, _ in steps[first_missing + 1:] if (root / name).exists()]
    if future:
        raise ValueError("发现越过当前阶段提前创建的规划产物：" + "、".join(future))
    name = steps[first_missing][1]
    return {
