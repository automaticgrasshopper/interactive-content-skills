...
开放形状只有`workflow_state.py`逐步放行并完成两版独立拓扑与匿名一次择优后才可进入验收；用户锁定完整形状时只允许一版拓扑，并跳过比较与内容复检。两种模式都必须到达`ACCEPT_PLANNING`、由规划门禁输出`PLANNING_ACCEPTED`，再由`accept_route.py CACHE_ROOT FORMAL_ROUTE.json`输出`ROUTE_ACCEPTED`并写出正式路线。完成后立即交给 UI 展示并结束，不得自动进入剧本或媒体生成。
