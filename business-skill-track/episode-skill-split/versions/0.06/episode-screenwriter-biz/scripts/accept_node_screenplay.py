...
def main() -> int:
    ...
    try:
        ...
        listed_characters = set(((draft.get("screenplay") or {}).get("关联角色") or []))
        for character in context.get("characters") or []:
            if character.get("name") in listed_characters and str(character.get("identity_anchor") or "") not in enhanced:
                raise ValueError(f"首次出场缺少冻结身份锚：{character.get('name')}")
