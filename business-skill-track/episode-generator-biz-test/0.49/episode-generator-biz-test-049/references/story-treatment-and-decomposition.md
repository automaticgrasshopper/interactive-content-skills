...
该故事只能从已通过的`mainline-story-input.json`生成。六组输入的选择和写作标准以`mainline-story-writing.md`为准；本文件从故事冻结后才进入上下文，只负责原文切片、裂缝审计和支线生长，不得反向要求故事预制节点或段尾选择。

主线是全图最完整的一条原始路线，不是最短路线，也不是最长路线的上限。后续允许其他路线与它等长或更长，只要因果与情绪任务尚未完成。冻结后任何支线都不得改写主线已经发生的因果。
...
独立小结局不等于死亡或灾难。动作使人物放弃核心目标、主动离开主要场域、错过不可恢复的时机、转向与本故事无关的生活，或让关系与资源状态不再提供合理回归动力时，当前故事已经进行不下去，必须判为`route_ended=true`、`ending_scope=minor`。不得为了让它回到主线而强行巧遇、回心转意或取消已发生的偏离。每次初次生成至少采用2个这样的独立小结局动作；它们不占主要结局预算，且必须来自至少两个不同的真实决定裂缝，不能在同一裂缝增加动作、用同一结果改名或在拓扑末端补数。自然扫描不足时退回线性主线重新设计真实决定情境。
...
`mainline_path`必须是一条真实的入口到正式结局路线，使冻结主线在全图中可辨认。`paths`必须精确枚举全部入口到结局路线；每条总分钟数等于沿途节点预计分钟数之和。该文件只用于路径一致性与内部记账，不从上游读取上限，也不反向压缩拓扑。正式节点数等于拓扑中唯一剧情节点集合大小，并写入拓扑合同的`resolved_node_count`。
