...
VERSION = "nextplay.episode-atomic-run.v3"
...
def refresh_progress(state: dict[str, Any]) -> dict[str, Any]:
    """Store the exact machine-readable resume point after every transition."""
    episodes = state.get("episodes") or {}
    order = state.get("episode_order") or []
    accepted = [
        episode_id
        for episode_id in order
        if (episodes.get(episode_id) or {}).get("state") == "EPISODE_ACCEPTED"
    ]
    next_episode = next(
        (
            episode_id
    ...
    state["total_count"] = len(order)
    state["next_episode_id"] = next_episode
    state["next_actions"] = available_actions(episodes[next_episode]) if next_episode else []
    state["active_leases"] = dict(episodes[next_episode].get("leases") or {}) if next_episode else {}
    state["status"] = "READY_FOR_CLOSURE" if next_episode is None else "IN_PROGRESS"
    return state


def persist(cache_root: Path, state: dict[str, Any]) -> None:
    atomic_write(state_path(cache_root), refresh_progress(state))
