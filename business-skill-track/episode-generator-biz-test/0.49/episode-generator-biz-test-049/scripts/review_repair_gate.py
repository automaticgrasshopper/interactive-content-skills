...
def merge(cache_root: Path, episode_id: str, finding_paths: list[Path]) -> dict[str, Any]:
    ...
    for source, path in zip(("dramatization", "cold-read"), finding_paths):
        item = read_json(path)
        if item.get("contract_version") == FINDINGS_VERSION:
            findings.append(item)
            continue
        if (
            item.get("episode_id") != episode_id
            or item.get("script_sha256") != sha_text(script)
            or item.get("issues") != []
        ):
            raise ValueError(f"{source}复检产物既不是有效问题单也不是当前PASS回执")
        findings.append({
    ...
    if {item.get("source") for item in findings} != SOURCES:
        raise ValueError("必须同时提供场面复检与独立冷读结果")
    errors: list[str] = []
