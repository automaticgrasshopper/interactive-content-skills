...
CONTRACT_VERSION = "nextplay.episode-split-handoff.v2"
PIPELINE_CONTRACT_VERSION = "nextplay.episode-split-run.v1"
...
def load_pipeline_state(path: Path, allowed_statuses: set[str]) -> dict[str, Any]:
    state = load_json(path)
    if not isinstance(state, dict) or state.get("contract_version") != PIPELINE_CONTRACT_VERSION:
        raise HandoffError("流水线状态合同不匹配")
    if state.get("status") not in allowed_statuses:
        raise HandoffError("当前流水线状态不允许创建或验证该交接")
    if not isinstance(state.get("run_id"), str) or not state["run_id"]:
        raise HandoffError("流水线状态缺少run_id")
    return state
...
def verify_manifest(
    ...
    if payload.get("pipeline_run_id") != pipeline_state.get("run_id"):
        raise HandoffError("交接未绑定当前流水线运行")
...
def create_planning(root: Path, output: Path, pipeline_state_path: Path) -> None:
    pipeline_state = load_pipeline_state(pipeline_state_path, {"planning_running"})
    ...
    payload = {
        ...
        "pipeline_run_id": pipeline_state["run_id"],
        "episode_ids": ids,
...
def verify_planning(root: Path, handoff: Path, pipeline_state_path: Path) -> dict[str, Any]:
    pipeline_state = load_pipeline_state(
        pipeline_state_path,
        {"planning_running", "planning_accepted", "writing_issued", "writing_running", "writing_accepted", "validation_issued", "validation_running", "deliverable_accepted"},
    )
    payload = load_json(handoff)
    if not isinstance(payload, dict):
        raise HandoffError("规划交接不是对象")
    verify_manifest(root, payload, "planning", pipeline_state)
...
def create_writing(root: Path, planning_handoff: Path, output: Path, pipeline_state_path: Path) -> None:
    pipeline_state = load_pipeline_state(pipeline_state_path, {"writing_running"})
    planning = verify_planning(root, planning_handoff, pipeline_state_path)
...
def verify_writing(
    root: Path,
    planning_handoff: Path,
    writing_handoff: Path,
    pipeline_state_path: Path,
) -> None:
    pipeline_state = load_pipeline_state(
        pipeline_state_path,
        {"writing_running", "writing_accepted", "validation_issued", "validation_running", "deliverable_accepted"},
    )
    planning = verify_planning(root, planning_handoff, pipeline_state_path)
    payload = load_json(writing_handoff)
    if not isinstance(payload, dict):
        raise HandoffError("写作交接不是对象")
    verify_manifest(root, payload, "writing", pipeline_state)
...
def main() -> int:
    ...
    for command in ("planning-create", "planning-verify", "writing-create", "writing-verify"):
        ...
        sub.add_argument("--pipeline-state", type=Path, required=True)
    ...
    try:
        if args.command == "planning-create":
            create_planning(root, args.output.resolve(), args.pipeline_state.resolve())
        elif args.command == "planning-verify":
            verify_planning(root, args.planning_handoff.resolve(), args.pipeline_state.resolve())
        elif args.command == "writing-create":
            create_writing(root, args.planning_handoff.resolve(), args.output.resolve(), args.pipeline_state.resolve())
        else:
            verify_writing(root, args.planning_handoff.resolve(), args.writing_handoff.resolve(), args.pipeline_state.resolve())
