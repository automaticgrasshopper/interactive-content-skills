...
def validate(path: Path, stage_two_input: Path | None = None) -> list[str]:
    ...
    if not isinstance(scale, dict) or set(scale) != SCALE_FIELDS:
        ...
        if stage_two_input is not None:
            try:
                stage_two_text = stage_two_input.read_text(encoding="utf-8")
                match = re.search(r"(\d+)\s*(?:至|到|[-–—~～])\s*(\d+)\s*分钟", stage_two_text)
                if not match:
                    issues.append("阶段二输入缺少可确定的单次游戏时长上限")
                elif values.get("route_duration_limit_minutes") != int(match.group(2)):
                    issues.append("路线时长上限未绑定阶段二输入")
