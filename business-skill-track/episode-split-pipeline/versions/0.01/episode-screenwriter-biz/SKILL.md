...
description: "当 episode-branch-planner-biz 已生成并验证 planning-handoff.json，或已有冻结拓扑和梗概且用户只要求修改某集剧情表达、动作或对白时，使用本 Skill。它逐集完成连续故事适配、Screenwriter原稿和原稿回执，不修改拓扑与梗概，不做Enhancer增强，不返回正式业务结果；全部原稿通过后必须在同一任务中继续使用 episode-script-validator-biz。"
---
...
把冻结拓扑与全体梗概逐集写成可表演的Screenwriter原稿。每集必须完成适配材料、连续写作输入、原稿和原稿回执，全部通过后生成不可变的`writing-handoff.json`并立即进入剧本校验阶段。
...
```bash
python3 scripts/stage_handoff.py writing-create <缓存目录> --planning-handoff <缓存目录>/planning-handoff.json --output <缓存目录>/writing-handoff.json
python3 scripts/stage_handoff.py writing-verify <缓存目录> --planning-handoff <缓存目录>/planning-handoff.json --writing-handoff <缓存目录>/writing-handoff.json
```

只有实际取得`WRITING_ACCEPTED`和`WRITING_HANDOFF_PASS`才完成写作阶段。随后立即加载`episode-script-validator-biz`继续同一任务；不得等待用户回复或提前返回部分剧本。
