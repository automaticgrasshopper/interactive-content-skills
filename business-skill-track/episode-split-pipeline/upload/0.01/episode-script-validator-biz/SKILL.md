...
description: "当 episode-screenwriter-biz 已生成并验证 writing-handoff.json，或已有冻结原稿且用户要求增强复写、检查、修复剧本表达与普通话话茬时，使用本 Skill。它逐集执行Enhancer增强、确定性结构验收、场面复检、独立冷读和受控局部修复，最后沿用原 episode-generator-biz 九字段合同生成正式业务JSON、完成凭证与交接文件。缺少已验证的规划或写作交接时不得执行。"
---
...
组装器只能映射已经验证的增强稿，不得摘要、压缩或新增正文。工程接收层必须自行取得独立一行`DELIVERABLE_ACCEPTED`才允许完成和投影；`STRUCTURE_PASS`不是完成证明。
