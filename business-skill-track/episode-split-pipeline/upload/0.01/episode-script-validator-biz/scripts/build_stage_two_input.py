#!/usr/bin/env python3
"""Build a stage-two generation view without upstream node-count suggestions."""
...
def from_sources(cache_root: Path) -> dict[str, Any]:
    ...
    if "node_count_hint" in json.dumps(packet, ensure_ascii=False):
...
def main() -> int:
    ...
    try:
        ...
        print(f"FAIL: {error}")
        return 1
    print("PASS: stage-two generation view excludes upstream node-count suggestions")
    return 0
