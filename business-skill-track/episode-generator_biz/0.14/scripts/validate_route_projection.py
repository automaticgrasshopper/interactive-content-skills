...
def validate(business: dict[str, Any], route: dict[str, Any]) -> list[str]:
    ...
    expected_choice_ids = [episode["选择节点"]["选择节点编号"] for episode in episodes if episode["选择节点"]["是否有选择问题"] is True]
    ...
    if route.get("settings", {}).get("episode_count") != len(expected_node_order):
        issues.append("settings.episode_count必须等于网页实际可视节点总数")
    ...
    for episode in episodes:
        ...
        if has_choice:
            ...
            if not isinstance(choice_node, dict):
                ...
                continue
            if choice_node.get("node_type") != "video":
                issues.append(f"{choice_id}/当前网页兼容载体node_type必须为video")
