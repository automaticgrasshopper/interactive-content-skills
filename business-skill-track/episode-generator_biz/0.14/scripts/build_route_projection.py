...
def build_route(
    ...
    for episode in episodes:
        ...
        visible_index += 1
        choice = episode["选择节点"]
        if choice["是否有选择问题"] is True:
            choice_route_refs[choice["选择节点编号"]] = f"episode-{visible_index:03d}"
    ...
    for logical_type, episode in visible_order:
        ...
        if logical_type == "choice":
            ...
            for index, option in enumerate(options, 1):
                ...
                target_ref = episode_route_refs[option["目标互动节点编号"]]
                edges.append(
                    {
                        "edge_ref": edge_ref,
                        ...
                        "metadata": {
                            ...
                            "target_business_episode_ref": option["目标互动节点编号"],
                        },
                    }
                )
    ...
    route.setdefault("settings", {})["episode_count"] = len(nodes)
    return route
...
def main() -> int:
    ...
    choice_count = sum(
        node.get("metadata", {}).get("logical_node_type") == "choice"
        for node in route["data"]["nodes"]
    )
    print(
        f"PASS: projected {episode_count} video episode nodes + "
        f"{choice_count} independent logical choice nodes"
    )
    return 0
