...
FIELDS = ["分集编号", "分集标题", "分集数", "选择数", "分集剧本", "剧本分析", "关联角色", "关联场景", "关联道具", "是否结局", "互动节点"]
...
def validate_episode(
    ...
    if re.fullmatch(r"\d+", blocks["选择数"]) is None:
        issues.append(f"选择数必须为非负整数：{node_id}")
    elif int(blocks["选择数"]) != expected_choice_count:
        issues.append(
            f"选择数错误：{node_id}期望{expected_choice_count}，实际{blocks['选择数']}"
        )
...
def build_structure(nodes: dict[str, dict[str, object]], synopses: dict[str, str]) -> str:
    ...
    for node_id, node in nodes.items():
        ...
        episode_type = "分支剧集" if node["choices"] else node["interaction"]
        parts.append(
...
def build_flowchart(nodes: dict[str, dict[str, object]]) -> str:
    ...
    for node_id, node in nodes.items():
        ...
        parts.append(f'<g data-node-id="{node_id}" data-node-type="剧集节点"><rect x="40" y="{y}" width="320" height="60" rx="10" fill="{fill}" stroke="#344054"/>')
