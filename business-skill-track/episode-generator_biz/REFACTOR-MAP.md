...
- 通用验证引擎同步升级为 `v0.1.52`；旧逐集质量回执仍按正文和 Reference 指纹判断，不因版本号单独失效。
...
- 当前业务协作快照：`0.04`
- 唯一工程字段合同：`0.04/references/business-interface.md`
- 精确内容结构：`0.04/references/episode-output-schema.md`
- 当前通用验证引擎：`v0.1.52`
...
| 十一字段及子字段、分集数、选择数、双节点投射、前后节点、结局、场次格式 | `validate_business_output.py` |
| 未授权分集不变、对白范围内非台词不变、拓扑不变 | `validate_business_output.py` |
| 三份公开文件确定性投射与整组事务提交 | `references/technical-handoff.md` 和技术 adapter |

## 输出字段锁

`0.02`起按网页 `_biz` 共同格式，`0.04`继续保持正式结果顶层只含：

- `分集结构`：派生交付拓扑，明确区分剧集节点和选择节点；分支固定投射为`剧集 -> 选择 -> 目标剧集`。
- `分集列表`：每集严格使用十一字段及全部子字段；`分集数`等于剧集节点总数，`选择数`等于选择节点总数，并在所有分集对象中一致。
