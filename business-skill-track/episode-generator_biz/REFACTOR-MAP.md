...
- 当前业务协作快照：`0.06`
- 唯一工程字段合同：`0.06/references/business-interface.md`
- 精确内容结构：`0.06/references/episode-output-schema.md`
- 当前通用验证引擎：`v0.1.53`
...
| `分集结构`拒绝`choice-xxx`与`选择节点`重复物化 | `validate_business_output.py` |
| 未授权分集不变、对白范围内非台词不变、拓扑不变 | `validate_business_output.py` |
| 三份公开文件确定性投射与整组事务提交 | `references/technical-handoff.md` 和技术 adapter |

## 输出字段锁

`0.02`起按网页 `_biz` 共同格式，`0.06`继续保持正式结果顶层只含：

- `分集结构`：只保存剧集节点及真实后继，不重复保存由互动派生的选择对象。
- `分集列表`：每集严格使用十一字段及全部子字段；`分集数`等于剧集节点总数，`选择数`等于有效互动节点数，并在所有分集对象中一致。前端从互动节点一次性生成可视选择节点。
