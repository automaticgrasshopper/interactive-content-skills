...
- 当前业务协作快照：`0.09`
- 唯一工程字段合同：`0.09/references/business-interface.md`
- 精确内容结构：`0.09/references/episode-output-schema.md`
- 当前通用验证引擎：`v0.1.53`
...
| 每个分集恰好一个互动节点、同一来源恰好封装一个选择节点 | `validate_business_output.py` |
| 未授权分集不变、对白范围内非台词不变、拓扑不变 | `validate_business_output.py` |
| 三份公开文件确定性投射与整组事务提交 | `references/technical-handoff.md` 和技术 adapter |

## 输出字段锁

`0.09`按 Sheet3 最新格式，正式结果顶层只含`分集列表`。每集严格使用十一字段及全部子字段：`分集数`等于互动节点总数并在所有分集对象中一致；`互动节点`只保存互动身份与后接选择关系；`选择节点`只封装问题、选项文字与目标互动节点。`选择数`和重复的`分集结构`不再属于正式业务字段。
