...
- `0.17`：不改变九字段、创作流程或路线规则；把“完整流程通过”升级为可执行提交事务。业务组装器在情绪脊、冻结拓扑、逐集正文、台词与独立复检、首次出场、用户意图、修改范围和公开三文件全部通过后，才写业务 JSON 并生成私有完成凭证；路线投影器必须重跑完整门禁、重建并逐值核对业务 JSON、验证完成凭证。结构正确但手写、过期或只经过局部校验的 JSON 不再能够生成路线。内部失败只返修并重跑，对外强制占位仍只允许`正在生成中`；不覆盖、不合并`0.16`。
...
- 当前业务协作快照：`0.17`
- 唯一工程字段合同：`0.17/references/business-interface.md`
- 精确内容结构：`0.17/references/episode-output-schema.md`
- 路线投影合同：`0.17/references/route-projection-contract.md`
- 当前通用验证引擎：`v0.1.53`
...
| 完整流程重建、完成凭证、手写或过期 JSON 旁路拒绝 | `assemble_business_output.py`、`completion_gate.py`、`build_route_projection.py` |

## 输出字段锁

`0.17`正式结果顶层只含`分集列表`。每集严格使用九个一级字段：`分集编号`、`分集标题`、`分集剧本`、`剧本分析`、`关联角色`、`关联场景`、`关联道具`、`是否结局`、`互动节点`。选择问题、选项文字、目标分集和默认下一分集只在来源分集的`互动节点`中封装一次；路线提案由内置投影器确定性派生独立选择卡。`分集数`、`选择数`、一级`选择节点`和重复的`分集结构`都不属于正式业务字段。
