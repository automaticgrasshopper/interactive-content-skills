...
- `route.data.nodes`同时含剧情节点与选择节点。剧情节点数量、编号和相对顺序必须与`分集列表`完全一致；每个有效选择紧邻放在来源剧情节点之后。
...
- 有选择时，剧情节点用一条`default`边连接唯一独立`choice`节点；该`choice`节点固定`interaction.has_interaction=true`，填写唯一选择编号、问题和选择边引用，每个选项从选择节点生成一条`choice`边。
