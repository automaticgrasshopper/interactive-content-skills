...
def validate(
    ...
    for index, item in enumerate(structures):
        ...
        if node_type == "剧集节点":
            ...
            if item["是否结局"] is False and item["类型"] != "剧情节点":
                issues.append(f"{node_id}/非结局剧集类型必须为剧情节点")
    ...
    choice_sources = [item["来源分集编号"] for item in choice_structure_by_id.values()]
    if len(choice_sources) != len(set(choice_sources)):
        issues.append("同一来源剧集被重复物化为多个选择节点")
    ...
    for index, item in enumerate(episodes):
        ...
        if is_branch:
            if structure["类型"] != "剧情节点":
                issues.append(f"{node_id}/带选择的源剧集仍必须是剧情节点")
