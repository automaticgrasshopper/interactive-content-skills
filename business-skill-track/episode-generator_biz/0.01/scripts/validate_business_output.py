#!/usr/bin/env python3
"""Validate the public episode-generator business JSON contract."""
...
CONTRACT = "episode-generator.business-output.v1"
TOP_FIELDS = [
    "合同版本", "状态", "动作", "完整业务内容", "本轮变更",
    "保持不变", "工具结果", "待确认问题", "警告", "错误",
]
CONTENT_FIELDS = ["分集结构", "分集列表"]
STRUCTURE_FIELDS = ["分集编号", "分集标题", "类型", "是否结局", "单集梗概", "后续节点编号列表"]
EPISODE_FIELDS = ["分集编号", "分集标题", "分集剧本", "剧本分析", "关联角色", "关联场景", "关联道具", "是否结局", "互动节点"]
...
TOOL_FIELDS = ["门禁", "状态"]
REQUIRED_GATES = {"情绪脊与拓扑", "逐集质量", "角色与首次出场", "九字段与全剧"}
STATUSES = {"完成", "部分完成", "待补充", "失败"}
ACTIONS = {"创建", "修改", "重做", "评审", "恢复"}
...
def validate(
    ...
    if not exact_keys(data, TOP_FIELDS, "顶层", issues):
        return issues
    if data["合同版本"] != CONTRACT:
        issues.append(f"合同版本错误：{data['合同版本']}")
    if data["状态"] not in STATUSES:
        issues.append("状态值非法")
    if data["动作"] not in ACTIONS:
        issues.append("动作值非法")
    for field in ("本轮变更", "保持不变", "待确认问题", "警告", "错误"):
        string_list(data[field], field, issues)
    if data["状态"] == "待补充" and len(data["待确认问题"]) != 1:
        issues.append("待补充时必须且只能有一个待确认问题")
    content = data["完整业务内容"]
    if content is None:
        if data["状态"] == "完成":
            issues.append("完成状态不能缺少完整业务内容")
        return issues
    if not exact_keys(content, CONTENT_FIELDS, "完整业务内容", issues):
        return issues
    tools = data["工具结果"]
    if not isinstance(tools, list):
        issues.append("工具结果必须是数组")
        tools = []
    gates: dict[str, str] = {}
    for index, item in enumerate(tools):
        if not exact_keys(item, TOOL_FIELDS, f"工具结果/{index}", issues):
            continue
        if not all(isinstance(item[field], str) and item[field].strip() for field in TOOL_FIELDS):
            issues.append(f"工具结果/{index}字段必须为非空字符串")
            continue
        if item["门禁"] in gates:
            issues.append(f"工具结果门禁重复：{item['门禁']}")
        gates[item["门禁"]] = item["状态"]
    if data["状态"] == "完成":
        required = set(REQUIRED_GATES)
        if data["动作"] in {"修改", "恢复"}:
            required.add("修改范围")
        if preflight:
            required.discard("九字段与全剧")
            required.discard("修改范围")
        missing = sorted(required - set(gates))
        failed = sorted(name for name in required & set(gates) if gates.get(name) != "通过")
        if missing:
            issues.append(f"完成状态缺少门禁：{missing}")
        if failed:
            issues.append(f"完成状态存在未通过门禁：{failed}")

    structures = content["分集结构"]
    episodes = content["分集列表"]
    ...
    for index, item in enumerate(structures):
        ...
        if not isinstance(node_id, str) or EPISODE_ID.fullmatch(node_id) is None:
            issues.append(f"分集结构/{index}/分集编号非法")
            continue
        if node_id in structure_by_id:
            issues.append(f"分集结构编号重复：{node_id}")
            continue
        ...
        targets = episode_ids(item["后续节点编号列表"], f"{node_id}/后续节点编号列表", issues)
        order.append(node_id)
        successors[node_id] = targets
        structure_by_id[node_id] = item
        if item["是否结局"] is True:
            endings.add(node_id)
    expected_order = [f"episode-{index:03d}" for index in range(1, len(order) + 1)]
    if order != expected_order:
        issues.append(f"分集编号或顺序不连续：{order}")
    predecessors = validate_graph(order, successors, endings, issues)
    ...
    for index, item in enumerate(episodes):
        if not exact_keys(item, EPISODE_FIELDS, f"分集列表/{index}", issues):
            continue
        node_id = item["分集编号"]
        if not isinstance(node_id, str) or node_id not in structure_by_id:
        ...
        structure = structure_by_id[node_id]
        ...
        if prev != predecessors.get(node_id, []):
            issues.append(f"{node_id}/前置节点与拓扑不一致")
        if nxt != successors[node_id]:
        ...
        if is_branch:
            ...
            if set(targets) != set(successors[node_id]):
                issues.append(f"{node_id}/选项目标与拓扑后继不一致")
            if not isinstance(question, str) or not question.strip():
                issues.append(f"{node_id}/选择问题为空")
            if default_next not in successors[node_id]:
            ...
            expected_default = "无" if node_id in endings else (successors[node_id][0] if len(successors[node_id]) == 1 else "无")
            if default_next != expected_default:
                issues.append(f"{node_id}/默认下一集错误")

    if list(episode_by_id) != order:
    ...
    if baseline is not None:
        base_content = baseline.get("完整业务内容") if isinstance(baseline, dict) else None
        if not isinstance(base_content, dict):
            issues.append("修改基线缺少完整业务内容")
        else:
            if base_content.get("分集结构") != structures:
                issues.append("局部修改期间冻结拓扑或结构发生变化")
            base_episodes = {
                item.get("分集编号"): item
                for item in base_content.get("分集列表", [])
...
def main() -> int:
    ...
    parser.add_argument(
        "--preflight",
        action="store_true",
        help="Validate business content before final gate receipts are inserted",
    )
    ...
    if issues:
        ...
        for issue in issues:
            print(f"- {issue}")
        return 1
    count = len(data["完整业务内容"]["分集列表"])
    print(f"PASS: {count} episodes; business contract, topology, fields, assets, endings, gates, and change scope verified")
    return 0
