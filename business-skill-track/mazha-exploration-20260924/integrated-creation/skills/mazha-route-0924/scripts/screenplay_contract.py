...
def format_issues(script: str) -> list[str]:
    issues = []
    lines = script.splitlines()
    dialogue = re.compile(r'[^：:【】（）\n]+（[^（）\n]+）：“[^“”\n]+”')
    if not lines or SCENE.fullmatch(lines[0]) is None:
        issues.append("正文必须从固定三段场次标题开始")
    if any(not line.strip() for line in lines):
        issues.append("正文不得有空行")
    if any(token in script for token in (r'\n', '/n', '```')):
        issues.append("正文不得含字面换行或代码围栏")
    for line in lines:
        if line.startswith('【') and SCENE.fullmatch(line) is None:
            issues.append("场次标题格式错误")
        if re.match(r'^\s*(?:#{1,6}\s|[-*]\s|\d+\.\s)', line):
            issues.append("正文不得含 Markdown 标题或列表")
        # A speech label at line start is distinguishable from narration such
        # as 他看见墙上写着：“回家”. The latter needs author review, not guessing.
        if re.match(r'^[^：:【】\s]{1,40}[：:]\s*[“"]', line) and dialogue.fullmatch(line) is None:
            issues.append("对白须独占一行并使用人物（情绪或语气）：引号台词格式")
...
def interaction_issues(node: dict) -> list[str]:
    value = node.get("互动节点")
    fields = {"是否为分支节点", "是否有选择问题", "选择问题", "选项列表", "默认下一分集编号"}
    if not isinstance(value, dict) or set(value) != fields:
        return ["本集互动材料字段不完整，不能省略分支准备投影"]
    is_choice = value["是否为分支节点"]
    if not isinstance(is_choice, bool) or value["是否有选择问题"] is not is_choice:
        return ["互动标志必须一致且为布尔值"]
    options = value["选项列表"]
    if not isinstance(options, list) or not isinstance(value["选择问题"], str):
        return ["选择问题和选项类型错误"]
    if is_choice:
        if len(options) < 2 or not value["选择问题"].strip():
            return ["分支来源缺少问题或两个真实选项"]
        keys = {"选项编号", "选项文字", "目标分集编号"}
        if any(not isinstance(o, dict) or set(o) != keys or not all(isinstance(v,str) and v.strip() for v in o.values()) for o in options):
            return ["互动选项字段错误"]
        if len({o["目标分集编号"] for o in options}) != len(options):
            return ["互动选项必须指向不同的立即后果"]
    elif options or value["选择问题"]:
        return ["普通剧情的互动材料不应含选择"]
    return []
...
def validate(route: Any, patch: Any, *, require_accepted: bool) -> list[str]:
    ...
    if node is None:
        ...
        return issues
    issues.extend(interaction_issues(node))
    ...
    if not exact(script_block, {"完整剧本"}, "分集剧本", issues):
        ...
        issues.extend(format_issues(script))
