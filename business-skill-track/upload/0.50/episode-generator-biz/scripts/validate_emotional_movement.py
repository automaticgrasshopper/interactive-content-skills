...
def validate(path: Path, stage_two_input: Path | None = None) -> list[str]:
    issues: list[str] = []
    try:
    ...
    if not isinstance(scale, dict) or set(scale) != SCALE_FIELDS:
        ...
        if scale.get("source") != "causal-capacity":
            issues.append("scale.source必须是causal-capacity")
        if stage_two_input is not None:
            try:
                stage_two = json.loads(stage_two_input.read_text(encoding="utf-8"))
                story = stage_two.get("story") if isinstance(stage_two, dict) else None
                if isinstance(story, dict) and "duration" in story:
                    issues.append("阶段二输入不得包含上游推荐时长")
            except (OSError, json.JSONDecodeError) as error:
                issues.append(f"阶段二输入不可用：{error}")
...
def main() -> int:
    ...
    parser.add_argument("--stage-two-input", type=Path)
    args = parser.parse_args()
    issues = validate(args.movement, args.stage_two_input)
    if issues:
        print("FAIL")
        for issue in issues:
            print(f"- {issue}")
        return 1
    print("PASS: unnumbered emotional movement and causal path envelope are valid")
    return 0
