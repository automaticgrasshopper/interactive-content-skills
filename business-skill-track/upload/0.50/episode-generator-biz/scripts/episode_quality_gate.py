...
from stage_two_acceptance import receipt_path as stage_two_receipt_path, verify_binding as verify_stage_two
...
REVIEW_DIR = "episode-quality-receipts"
...
def comprehension_declares_failure(answer: str) -> bool:
    """Reject a reviewer disclaimer, not quoted story consequences."""
    compact = answer.strip()
    for prefix in ("结论：", "答案："):
        if compact.startswith(prefix):
            compact = compact[len(prefix):].lstrip()
    return any(compact.startswith(marker) for marker in COMPREHENSION_FAILURE_MARKERS)
...
def read_synopsis(cache_root: Path, episode_id: str) -> tuple[Path, dict[str, Any]]:
    ...
    # Titles are user-controlled when explicitly supplied, so this gate only
    # rejects an empty title.  Generated-title length is a writing rule, not a
    # reason to block an otherwise valid frozen title.  Synopsis and conflict
    # remain subject to the readability floor.
    if not str(data.get("title") or "").strip():
        raise ValueError(f"单集梗概草案缺少可读内容：{episode_id}/title")
    for key in ("synopsis", "conflict"):
...
def build_packet(cache_root: Path, episode_id: str) -> dict[str, Any]:
    _assert_no_unsealed_work(cache_root, episode_id)
    _assert_predecessors_sealed(cache_root, episode_id)
    synopsis_errors = verify_stage_two(cache_root)
    if synopsis_errors:
...
def validate_review(packet: dict[str, Any], review: dict[str, Any]) -> list[str]:
    ...
    for key, label in COMPREHENSION_FIELDS.items():
        ...
        elif comprehension_declares_failure(answer):
            errors.append(f"理解门未通过：{label}")
        comprehension_proofs.append(proof)
    if len([proof for proof in comprehension_proofs if proof]) != len(
        set(proof for proof in comprehension_proofs if proof)
    ):
        errors.append("理解门四项不得重复使用同一正文证据")
    ...
    for check in REQUIRED_CHECKS:
        ...
        elif check == BOUNDARY_CHECK:
            fields = ("script_stop_proof", "explanation", "successor_proof")
            if any(len(str(item.get(field) or "").strip()) < 4 for field in fields):
                errors.append(f"复检证据不完整：{check}")
            elif str(item["script_stop_proof"]).strip() not in str(packet["script"]):
                errors.append(f"当前集停止证据不在完整剧本中：{check}")
            elif packet["successor_synopses"]:
                successor_corpus = "\n".join(
                    str(successor.get("synopsis") or "")
                    for successor in packet["successor_synopses"]
                )
                if str(item["successor_proof"]).strip() not in successor_corpus:
                    errors.append(f"后续独占动作证据不在直接后续梗概中：{check}")
            elif str(item["successor_proof"]).strip() != "无后续分集":
                errors.append(f"结局集边界证据错误：{check}")
    ...
    if review.get("synopsis_set_sha256") != packet["synopsis_set_sha256"]:
        errors.append("复检未绑定当前冻结梗概集合")
    if review.get("story_material_sha256") != packet["story_material_sha256"]:
        errors.append("复检未绑定当前连续故事材料与停止边界")
    return errors
...
def _raw_verify_receipt(cache_root: Path, episode_id: str) -> list[str]:
    ...
    for key in (
        ...
        "user_intent_contract_sha256", "synopsis_set_sha256",
        "story_material_sha256", "required_checks",
    ):
...
def _assert_no_unsealed_work(cache_root: Path, episode_id: str) -> None:
...
def build_packet_unchecked(cache_root: Path, episode_id: str) -> dict[str, Any]:
    ...
    material_path = cache_root / "episode-story-materials" / f"{episode_id}.json"
    if not material_path.is_file():
        raise ValueError(f"缺少当前连续故事材料：{episode_id}")
    material = json.loads(material_path.read_text(encoding="utf-8"))
    if not isinstance(material, dict) or material.get("episode_id") != episode_id:
        raise ValueError(f"当前连续故事材料合同或编号错误：{episode_id}")
    successor_synopses = []
    for target in synopsis_data["successors"]:
        _, successor = read_synopsis(cache_root, target)
        successor_synopses.append({
            "episode_id": target,
            "title": successor["title"],
            "synopsis": successor["synopsis"],
    ...
    stage_two = json.loads(stage_two_receipt_path(cache_root).read_text(encoding="utf-8"))
    return {
        "packet_version": "episode-quality-gate-v4",
        ...
        "synopsis_set_sha256": stage_two["files"]["synopsis-set-review.json"],
        "story_material_sha256": sha256(
            json.dumps(material, ensure_ascii=False, sort_keys=True, separators=(",", ":"))
        ),
        "required_checks": REQUIRED_CHECKS,
        "reference": reference,
        "synopsis": artifact_synopsis,
        "stop_boundary": str(material.get("stop_boundary") or ""),
        "successor_synopses": successor_synopses,
        "script": script,
    }
...
def seal(cache_root: Path, episode_id: str, review_path: Path) -> Path:
    ...
    receipt = {
        ...
        "synopsis_set_sha256": packet["synopsis_set_sha256"],
        "story_material_sha256": packet["story_material_sha256"],
        "required_checks": REQUIRED_CHECKS,
...
def verify_project(cache_root: Path) -> list[str]:
    errors: list[str] = []
    errors.extend(verify_stage_two(cache_root))
...
def main() -> int:
    ...
    try:
        if args.command == "packet":
            packet_value = build_packet(args.cache_root, args.episode_id)
