...
def validate(
    ...
    for index, item in enumerate(episodes):
        ...
        if not isinstance(node_id, str) or EPISODE_ID.fullmatch(node_id) is None:
            ...
            continue
        if node_id in by_id:
            issues.append(f"分集编号重复：{node_id}")
            continue
        ...
        has_choice = interaction["是否有选择问题"] is True
        if not isinstance(interaction["是否为分支节点"], bool) or not isinstance(interaction["是否有选择问题"], bool):
            issues.append(f"{node_id}/节点标记必须为布尔值")
        if is_branch != has_choice:
            issues.append(f"{node_id}/分支标记与选择问题标记不一致")
        options = interaction["选项列表"] if isinstance(interaction["选项列表"], list) else []
        if not isinstance(interaction["选项列表"], list):
            issues.append(f"{node_id}/选项列表必须为数组")
        if has_choice:
            if not isinstance(interaction["选择问题"], str) or not interaction["选择问题"].strip():
            ...
            if interaction["默认下一分集编号"] not in targets:
                issues.append(f"{node_id}/默认下一分集不在选项目标中")
            if node_id == "episode-001":
                validate_opening_choice(script, issues)
        else:
            if interaction["选择问题"] not in {"", "无"} or options:
                issues.append(f"{node_id}/非选择节点含选择信息")
            expected_default = "无" if item["是否结局"] or len(successors[node_id]) != 1 else successors[node_id][0]
            if interaction["默认下一分集编号"] != expected_default:
                issues.append(f"{node_id}/默认下一分集编号错误")
    ...
    # validate_emotional_topology.py 在业务投射前强验证；最新版公开九字段
