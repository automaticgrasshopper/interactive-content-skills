...
def build_packet(cache_root: Path, episode_id: str) -> dict[str, Any]:
    ...
    synopsis_errors = verify_synopsis_set(cache_root)
    if synopsis_errors:
...
def build_packet_unchecked(cache_root: Path, episode_id: str) -> dict[str, Any]:
    ...
    if dramatization_errors:
        raise ValueError("场面展开尚未放行：" + "；".join(dramatization_errors))
    ...
    synopsis_set = synopsis_set_packet(cache_root, allow_scripts=True)
    return {
        ...
        "synopsis_set_sha256": synopsis_set["synopsis_set_sha256"],
