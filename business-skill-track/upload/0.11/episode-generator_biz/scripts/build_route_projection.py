...
def mentions(episode: dict[str, Any], refs: dict[str, str]) -> list[dict[str, str]]:
    ...
    for field, role in (("关联角色", "character"), ("关联场景", "scene"), ("关联道具", "item")):
        for name in episode[field]:
            ...
            result.append({"ref_id": refs[name], "role": role})
    return result
        ...
        nodes.append(
            {
                ...
                "interaction": {
                    "interaction_ref": choice["选择节点编号"] if has_choice else None,
                    "has_interaction": has_choice,
                    "question": choice["选择问题"] if has_choice else None,
                    "after_plot_beat": episode["分集剧本"]["单集梗概"] if has_choice else None,
                ...
                "timeline": {"clips": []},
                "playback": {
                    "mode": "interactive" if has_choice else "linear",
                    "auto_advance": not has_choice,
                    "default_next_edge_ref": (
                        None if has_choice or not successors else f"edge-{episode_id[-3:]}-01"
                    ),
                },
                "conditions": {"enter": []},
...
def main() -> int:
    ...
    try:
        ...
        print(f"FAIL: {error}")
        return 1
    print(f"PASS: projected {len(route['data']['nodes'])} episode nodes")
    return 0
