#!/usr/bin/env python3
"""Build the business JSON only after the complete v0.1.52 gates pass."""
...
def build_business_output(
    ...
    for node_id, node in nodes.items():
        ...
        if node["choices"]:
            choice_index += 1
            choice_id = f"choice-{choice_index:03d}"
        structures.append(
            {
                "节点编号": node_id,
                "节点类型": "剧集节点",
                "分集编号": node_id,
                "分集标题": str(node["title"]),
                "类型": "分支剧集" if node["choices"] else str(node["interaction"]),
                "是否结局": bool(node["ending"]),
                "单集梗概": synopsis,
                "来源分集编号": "",
                "选择问题": "",
                "选项列表": [],
                "后续节点编号列表": [choice_id] if choice_id else list(node["successors"]),
            }
        )
        if choice_id:
            ...
            structures.append(
                {
                    ...
                    "单集梗概": "",
                    "来源分集编号": node_id,
                    "选择问题": interaction["选择问题"],
                    "选项列表": interaction["选项列表"],
                    "后续节点编号列表": [
                        option["目标分集编号"] for option in interaction["选项列表"]
                    ],
                }
            )
    issues.extend(
...
def main() -> int:
    ...
    print(
        ...
        "v0.1.52 gates and business output contract verified"
    )
    return 0
