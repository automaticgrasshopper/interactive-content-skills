...
def validate(
    ...
    if structure:
        structure_ids = re.findall(r"^##\s+(episode-\d{3})", structure.read_text(encoding="utf-8"), re.MULTILINE)
        if structure_ids != list(nodes):
            issues.append("公开结构编号或顺序与拓扑不一致")
...
def main() -> int:
    ...
    try:
        ...
        issues = validate(nodes, args.structure, args.expected_endings, args.expected_formal, args.expected_failure)
