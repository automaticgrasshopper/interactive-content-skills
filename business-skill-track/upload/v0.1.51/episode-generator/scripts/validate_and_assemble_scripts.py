...
FIELDS = ["分集编号", "分集标题", "分集数", "分集剧本", "剧本分析", "关联角色", "关联场景", "关联道具", "是否结局", "互动节点"]
...
def validate_episode(
    ...
    if headings != FIELDS:
        return [f"十字段错误：{node_id}={headings}"], ""
    try:
    ...
    if is_branch != expected_branch or has_question != expected_branch:
        issues.append(f"分支布尔值与拓扑不一致：{node_id}")
    ...
    if expected_branch:
        ...
        if default_next not in expected_next:
            issues.append(f"默认下一集不在选择后继中：{node_id}")
    else:
        if numbers or option_texts or option_targets or option_block not in {"", "无"}:
            issues.append(f"非分支节点含选项：{node_id}")
        expected_default = "无" if expected_ending else (expected_next[0] if len(expected_next) == 1 else "无")
        if default_next != expected_default:
            issues.append(f"默认下一集错误：{node_id}={default_next} expected={expected_default}")
...
def build_flowchart(nodes: dict[str, dict[str, object]]) -> str:
    ...
    for source, node in nodes.items():
        y1 = 100 + index[source] * 120
        for edge_index, target in enumerate(node["successors"]):
            if target not in index:
                continue
            y2 = 40 + index[target] * 120
            control_x = 430 + edge_index * 45
            parts.append(f'<path data-edge="{source}->{target}" d="M200,{y1} C{control_x},{y1} {control_x},{y2} 200,{y2}" fill="none" stroke="#667085" stroke-width="2" marker-end="url(#arrow)"/>')
    for node_id, node in nodes.items():
        y = 40 + index[node_id] * 120
        fill = "#fee4e2" if node["ending"] else "#e0f2fe"
        parts.append(f'<g data-node-id="{node_id}"><rect x="40" y="{y}" width="320" height="60" rx="10" fill="{fill}" stroke="#344054"/>')
...
def main() -> int:
    ...
    print(f"PASS: {len(scripts)} episodes validated; three public files built and read back")
