...
def validate(
    ...
    for index, item in enumerate(structures):
        ...
        node_id = item["节点编号"]
        node_type = item["节点类型"]
        if node_type == "选择节点" or (isinstance(node_id, str) and node_id.startswith("choice-")):
            issues.append(f"分集结构/{index}/不得重复物化选择节点")
        if not isinstance(node_id, str) or EPISODE_ID.fullmatch(node_id) is None:
            issues.append(f"分集结构/{index}/节点编号非法")
            continue
        if node_id in episode_structure_by_id:
            issues.append(f"分集结构节点编号重复：{node_id}")
            continue
        if node_type != "剧集节点":
            issues.append(f"{node_id}/分集结构不得重复物化选择节点")
        ...
        if any(EPISODE_ID.fullmatch(target) is None for target in targets):
        ...
        creative_successors[node_id] = targets
        ...
        if item["来源分集编号"] != "" or item["选择问题"] != "" or item["选项列表"] != []:
            issues.append(f"{node_id}/分集结构不得重复携带互动内容")
    ...
    creative_predecessors = validate_graph(episode_order, creative_successors, endings, issues)
    ...
    for index, item in enumerate(episodes):
        ...
        if (
            ...
            issues.append(f"{node_id}/选择数必须为非负整数")
    ...
    choice_count = sum(
        ...
        if isinstance(item.get("互动节点"), dict)
        and item["互动节点"].get("是否为分支节点") is True
    )
    for node_id, item in episode_by_id.items():
        if item["选择数"] != choice_count:
            issues.append(f"{node_id}/选择数必须等于有效互动节点总数{choice_count}")
    total = len(endings)
