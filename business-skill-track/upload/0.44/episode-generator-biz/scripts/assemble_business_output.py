...
from completion_gate import make_handoff, make_receipt
...
BUSINESS_SKILL_VERSION = "episode-generator-biz/0.44"
...
def build_business_output(
    ...
    issues.extend(validate_revision(cache_root / "topology-draft-1.md", topology_path))
    issues.extend(verify_mainline(cache_root))
    issues.extend(verify_fissures(cache_root))
    issues.extend(verify_treatment(cache_root))
    issues.extend(validate_route_duration(
        cache_root / "route-duration.json",
        topology_path,
        cache_root / "stage-two-input.json",
    ))
    issues.extend(verify_synopsis_set(cache_root))
