#!/usr/bin/env python3
"""Validate a fresh full-graph scan before any user-directed local node edit."""
...
def validate(data: Any, request_bytes: bytes) -> tuple[list[str], dict[str, Any] | None]:
    ...
    operation_context = data.get("operation_context")
    if not isinstance(operation_context, dict) or set(operation_context) != OPERATION_CONTEXT_FIELDS:
        issues.append("当前画布操作内容字段错误")
        operation_context = {"title": "", "content": ""}
    elif not all(isinstance(operation_context.get(field), str) for field in OPERATION_CONTEXT_FIELDS):
        issues.append("当前画布操作标题和内容必须是字符串")
        operation_context = {"title": "", "content": ""}
    ...
    preliminary_targets = data.get("target_node_refs")
    target_ref_set = set(preliminary_targets) if string_list(preliminary_targets) else set()
    ...
    for index, node in enumerate(nodes_value):
        ...
        if not isinstance(title, str):
            issues.append(f"节点标题必须是字符串：{ref}")
        elif not title.strip() and not (operation in {"add", "generate"} and ref in target_ref_set):
            issues.append(f"非新增目标节点标题为空：{ref}")
    ...
    if operation in {"add", "generate", "update"}:
        if not isinstance(targets, list) or len(targets) != 1:
            ...
            if node and operation_context.get("title") != node.get("title"):
                issues.append("本轮操作标题与当前目标节点标题不一致")
    ...
    entered_title = bool(operation_context["title"].strip())
    entered_content = bool(operation_context["content"].strip())
    if entered_title and entered_content:
        input_state = "title_and_content"
    elif entered_title:
        input_state = "title_only"
    elif entered_content:
        input_state = "content_only"
    else:
        input_state = "empty"
    ...
    receipt = {
        ...
        "operation_title": operation_context["title"],
