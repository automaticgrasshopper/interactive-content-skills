...
2. 先建立`dramatization-plans/<episode-id>.json`：按梗概原文顺序完整拆出`setup`与`enact`拍，给每个`enact`写明开始状态、至少两个可见步骤、结束状态和真实状态变化。运行`dramatization_gate.py plan`，PASS前不得写正文。
...
6. 运行当前集结构门禁。
7. 结构门禁PASS后生成场面展开复检包，逐拍引用正文中的可见步骤与结果证据。存在未展开拍时只返修当前集；场面展开回执PASS前不得进入独立冷读。
...
每集通过必须同时满足：全体梗概集合已有当前有效回执；场面展开计划完整覆盖梗概并已绑定当前完整故事；九字段结构正确；前后节点、选择、结局和资产与冻结材料一致；正文非空且场次格式正确；梗概中的关键事件均有按序可见步骤与真实状态变化，未用梗概旁白代替戏；理解门四项有互不替代的正文证据；全部冷读项覆盖；问题为空；当前正文、reference、冻结梗概集合和用户意图指纹与回执一致。
