...
def validate(
    ...
    elif expected_endings is not None and total_endings != expected_endings:
        issues.append(f"结局数量错误：期望{expected_endings}，实际{total_endings}")
    ...
    if movement is not None and roots == ["episode-001"] and count == len(nodes):
        ...
        if not isinstance(scale, dict):
            ...
            path_min = scale.get("target_formal_path_length_min")
            path_max = scale.get("target_formal_path_length_max")
            ...
            for path in paths:
                ...
                if "正式结局" not in str(ending["interaction"]):
                    continue
                choice_count = sum(
                    1
                    for item in path
                    if "选择" in str(nodes[item]["interaction"])
                    and "结果" not in str(nodes[item]["interaction"])
                )
                if isinstance(path_min, int) and len(path) < path_min:
                    issues.append(f"正式结局路径短于目标下限：{path[-1]}={len(path)}<{path_min}")
                if isinstance(path_max, int) and len(path) > path_max:
                    issues.append(f"正式结局路径长于目标上限：{path[-1]}={len(path)}>{path_max}")
                if isinstance(interaction_min, int) and choice_count < interaction_min:
                    issues.append(
                        f"正式结局路径互动少于探索下限：{path[-1]}={choice_count}<{interaction_min}"
                    )
                if isinstance(interaction_max, int) and choice_count > interaction_max:
                    issues.append(
                        f"正式结局路径互动多于探索上限：{path[-1]}={choice_count}>{interaction_max}"
                    )
    return issues
