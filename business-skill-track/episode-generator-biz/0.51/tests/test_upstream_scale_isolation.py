...
def write_intent(root: Path, *, fixed_episode_count=None, constraints=None, request="请保留我明确提出的内容。") -> dict:
...
class MinimalInputTests(unittest.TestCase):
    ...
    def test_upstream_count_cannot_be_injected_as_user_constraint(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            constraint = {
            ...
            write_intent(root, fixed_episode_count=6, constraints=[constraint])
            _, _, issues = validate_user_intent_lock.validate_contract(root)
            self.assertTrue(any("不是 user-request.md 中的逐字原话" in issue for issue in issues))
            self.assertTrue(any("缺少用户明确固定N集" in issue for issue in issues))
    ...
    def test_fixed_episode_count_is_checked_only_against_finished_topology(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            contract = write_intent(root, fixed_episode_count=2, constraints=[constraint], request="必须固定为2集")
