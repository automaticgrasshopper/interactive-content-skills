...
2. 写当前集九字段正文到`episodes/<episode-id>.md`。
3. 单独提取台词，执行轻量台词复写并放回原位。
4. 首次出场角色出现时，立即把身份、关系、当前必要性三条逐字证据写入`character-introductions.json`。
5. 运行当前集结构门禁。
6. 生成当前集冷读包，按包内reference逐场完成理解门和七项复检。
7. 有问题时只返修当前集，再从第2步重跑；问题清空后封存回执。
8. 当前集结构门禁和回执都PASS，才处理下一集。
...
每集通过必须同时满足：九字段结构正确；前后节点、选择、结局和资产与冻结材料一致；正文非空且场次格式正确；理解门四项有正文证据；七项冷读全部覆盖；问题为空；普通话检查分别举证台词和叙述；当前正文、reference和用户意图指纹与回执一致。
