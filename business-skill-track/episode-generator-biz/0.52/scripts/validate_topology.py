...
def validate(
    ...
    for node_id, node in nodes.items():
        ...
        if is_choice:
            ...
            if len(targets) >= 3 and all(
                ...
                issues.append(f"禁止把三个及以上终点做成同一次选择的结局菜单：{node_id}")
        elif choices:
    ...
    if roots == ["episode-001"] and count == len(nodes):
        ...
        queued = {"episode-001"}
        display_queue = deque(["episode-001"])
        while display_queue:
            node_id = display_queue.popleft()
            display_order.append(node_id)
            for target in nodes[node_id]["successors"]:
                if target in nodes and target not in queued:
                    queued.add(target)
                    display_queue.append(target)
        expected_display_order = [f"episode-{index:03d}" for index in range(1, len(nodes) + 1)]
        if display_order != expected_display_order:
            issues.append(
                "编号未按从左到右、同层按分支从上到下的程序显示顺序："
                + " -> ".join(display_order)
            )
