...
def validate_review(packet_value: dict[str, Any], review: dict[str, Any]) -> list[str]:
    ...
    if not isinstance(volume, dict) or set(volume) != {"result", "reason", "recommendation"}:
        ...
        if result in {"story-longer-than-signal", "story-shorter-than-signal"}:
            if not isinstance(recommendation, str) or len(recommendation.strip()) < 8:
                issues.append("体量明显不匹配时必须提供中性调整建议")
        elif recommendation not in (None, ""):
            issues.append("体量一致时不得制造调整建议")
