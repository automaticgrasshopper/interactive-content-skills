    ...
    if not isinstance(scale, dict) or set(scale) != SCALE_FIELDS:
        ...
        for field in (
            ...
            if not isinstance(value, int) or isinstance(value, bool) or value < 1:
                ...
                values[field] = value
        if values.get("path_length_min", 0) > values.get("path_length_max", 0):
            issues.append("单路径体量区间倒置")
        if values.get("interaction_min", 0) > values.get("interaction_max", 0):
            issues.append("互动探索区间倒置")
        if values.get("interaction_max", 0) > values.get("path_length_max", 0):
            issues.append("互动上限不得超过单路径体量上限")
