...
- 写前覆盖规划与写后验收读取`references/causal-episode-writing.md`和`references/dramatization-completion.md`
- 真正撰写正文时只读取`references/screenplay-adaptation.md`、当前单集写作包与`references/chinese-dialogue-craft.md`
- 最终正文与台词复写完成后，才读取`references/character-appearance-validation.md`提取首次出场证据
...
1. 只携带已冻结的当前集梗概、当前节点、直接前置结果、仍有效的状态差异、相关人物与资产、世界规则和直接后续入口。情绪脊、PAD/VAD数值、路线为何成立的拓扑说明和全局情绪结算退出写作上下文，不得转写成叙述或台词。完整故事只允许检索与当前梗概直接对应的逐字来源，不得重新载入全文或借它越过当前梗概塞入后续事件。
2. 先建立`dramatization-plans/<episode-id>.json`并通过`dramatization_gate.py plan`。它只负责确认梗概中的重大结果都有可执行过程；计划通过后退出编剧上下文，不得成为正文段落、句式或举证模板。
3. 运行`build_episode_writing_context.py`，把当前集冻结事实转换成`episode-writing-contexts/<episode-id>.json`故事材料。真正写作时只使用该文件和写作references，不重新载入过程链原文件、完整故事、情绪脊、无关路线、未来正文或验收reference。
4. 按`screenplay-adaptation.md`连续完成三步：先私下判断本集由人物对峙、动作执行或情绪过渡中的哪一种主导并规划连续场面；再把故事材料写成完整可演剧本；最后做一次不改剧情的全场增强，改善具体感、动作连续、场景转接、人物表演和对白自然度。三步不新增正式业务字段，也不输出中间计划。
5. 把冻结梗概原样写入`episodes/<episode-id>.md`并放入增强后的最终正文。随后单独提取台词，锁定事实、动作和结果，按`chinese-dialogue-craft.md`恢复必要的接话、反驳、迟疑和关系指向，再放回原位。
6. 最终正文与台词复写完成后才读取首次出场验收规则并提取身份、关系、当前必要性三条逐字证据写入`character-introductions.json`。证据缺失时回到全场增强，改写角色进入现场的动作、称呼、接话或自然叙述；不得在成品中补插“某某是……”式登记句。所在地点和无身份含义的当前动作不能代替身份；单独叫名字不能代替关系，但称呼与紧邻的命令、配合、拒绝、保护或任务依赖可以共同证明当前关系。冻结背景关系只约束表演，不要求首次出场逐条口述。
7. 运行当前集结构门禁。
8. 结构门禁PASS后生成场面展开复检包。每个过程分别引用行动、阻力或核验、结果三处按序且不重复的正文证据；事件结束状态晚于全部过程。任何重大结果没有过程链，或过程只能靠一句旁白证明，均视为未展开。场面展开回执PASS前不得进入独立冷读。
9. 场面展开PASS后再换档到独立冷读。冷读只允许引用脚本生成的当前集质量复检包和独立复检reference，不得读取场面展开计划、写作笔记、作者计划、预期答案或后续剧情，随后验证理解门、梗概与正文一致及其余质量项。
10. 有问题时只返修当前集正文；若发现冻结梗概本身无法成立，停止阶段三并退回阶段二，重建受影响梗概集合及其回执，不能在正文中暗改梗概。
11. 正文变化后，场面展开回执与冷读回执同时失效，必须依次重建。两类回执都PASS且当前集`verify-one` PASS，才开始下一集正文。

### 当前集门禁

```bash
python3 scripts/dramatization_gate.py plan <缓存目录> <分集编号>
python3 scripts/build_episode_writing_context.py <缓存目录> <分集编号> --output <缓存目录>/episode-writing-contexts/<分集编号>.json
