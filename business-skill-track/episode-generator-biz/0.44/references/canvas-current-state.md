...
3. 将该次返回原样整理为`canvas-current-snapshot.json`，使用平台返回的不透明`node_ref`和`project_revision`，并把本轮操作记录中用户已经填写的标题和内容整理进`operation_context`，不自行臆造。
...
- `contract_version`：固定为`nextplay.canvas-current-snapshot.v2`。
- `scope`：固定为`full-graph`。
- `project_revision`：平台本次重读返回的revision，保持平台返回的非负整数或非空字符串类型。
...
- `operation_context`：本轮操作记录中的用户输入，严格含字符串`title`和`content`；未填写时使用空字符串。删除操作可在`title`中保留平台本轮明确返回的被删标题，不从旧快照补写。
- `nodes`：全图节点数组。每项严格含`node_ref`、`display_id`、`title`、`predecessors`、`successors`和`choices`；`choices`每项含`text`与`target_ref`。

合同版本0.44使用`nextplay.canvas-current-snapshot.v2`。新增或生成操作的唯一目标节点允许标题暂时为空，其他节点仍要求非空标题。验证回执同时记录`empty`、`title_only`、`content_only`或`title_and_content`四种输入状态，供局部编辑阶段选择对应话术。
