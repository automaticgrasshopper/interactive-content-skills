...
3. `story-treatment.json`在不改写主线的前提下记录支线生长、选择后果与全部结局。
4. `topology.md`把故事图拆成一节点一集；`route-duration.json`用分钟校验全部路线。
5. `episode-synopses/*.json`把每个节点冻结为可写梗概。
6. 分集正文只改编对应梗概，不反向改故事或拓扑。
...
`decision-fissure-audit.json`使用`nextplay.decision-fissure-audit.v1`。根对象必须逐字复制`stage-two-input.json`的`core_goal`并给出主线证据。从冻结主线开场扫描到结局，每个候选记录主线逐字位置、决定问题、至少两个可执行且互斥动作、即时后果、核心目标状态、路线是否结束、对照原始核心目标的解释和理由。`core_goal_status`只能是`continuing`、`achieved`、`failed`、`abandoned`或`unreachable`；除`continuing`外都必须令`route_ended=true`。

候选只能标记`adopt`或`reject-causally-insufficient`。不得用部分证据、人员安全或继续追查等代价路线新目标替换冻结核心目标。主线出现明确交易、交换、命令、停查或交出要求时，审计必须同时列出字面服从与拒绝；假意接受、拖延或讨价还价不能替代真实服从动作。只要任一动作在即时后果后已经让核心目标完成、失败、放弃或不可达，该裂缝必须`adopt`，不得被同位置更弱的快速汇合选择替代。

## 四、支线故事图合同

`story-treatment.json`使用`nextplay.episode-story-treatment.v3`：

```json
{
  "contract_version": "nextplay.episode-story-treatment.v3",
  "title": "故事标题",
  "mainline_sha256": "冻结mainline-story.json的规范化哈希",
  "decision_fissure_audit_sha256": "已通过裂缝审计的规范化哈希",
  "complete_story": "主线保持不变，并补写全部支线的连续因果故事",
  "choices": [
    {
