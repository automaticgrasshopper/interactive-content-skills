...
3. 在任何支线故事或拓扑之前生成`decision-fissure-audit.json`。逐字冻结`stage-two-input.json`中的`core_goal`，从主线开场扫描到结局，登记全部真实决定裂缝、当时可执行且互斥的动作、各动作即时后果，以及后果发生后原始核心目标是继续、完成、失败、放弃还是不可达；不得用“还能送部分材料”等新目标替换原始核心目标。主线出现明确交易、交换、命令或停查要求时，同一裂缝必须同时审计字面服从与拒绝动作，假意服从不能替代真实接受。原始目标不再继续时`route_ended`必须为真；含任一因果充分结束动作的裂缝必须采用，不能用同位置较弱的快速汇合选择替代。独立复检并封存`decision-fissure-review.json`。
4. 只读取冻结主线、通过的裂缝审计、未编号情绪运动和阶段二输入，生成`story-treatment.json`。保持主线逐事件不变，逐项投影全部已采用裂缝并生长支线，写清即时后果、持续差异、汇合条件与其他结局。支线可短于、等于或长于主线，可二级开扇或完全分线；只服从自己的因果、情绪运动与上游单次游戏时长上限。
...
6. 编号全部戏剧单位并生成正式`topology.md`。主线路径必须在拓扑中完整可识别并忠于冻结主线；支线只能从真实决策裂缝生长，不得反向改写主线。
