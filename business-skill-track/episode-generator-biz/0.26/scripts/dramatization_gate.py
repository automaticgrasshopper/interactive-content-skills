...
RECEIPT_VERSION = "nextplay.episode-dramatization-review.v3"
...
def build_packet(cache_root: Path, episode_id: str) -> dict[str, Any]:
    ...
    if near_copied:
        raise ValueError("关键梗概被近似搬入叙述，必须补出演出过程：" + "、".join(near_copied))
    summaries = author_summary_passages(script)
    if summaries:
        raise ValueError("正文出现作者摘要，必须改成动作、回应与现场后果：" + "｜".join(summaries))
    ...
    return {
        ...
        "episode_artifact_sha256": sha256_text(artifact),
        "full_script_sha256": sha256_text(script),
...
def validate_review(packet: dict[str, Any], review: dict[str, Any]) -> list[str]:
    errors: list[str] = []
    for key in (
        "plan_sha256", "episode_artifact_sha256", "full_script_sha256",
        "synopsis_sha256", "story_treatment_sha256", "reference_sha256",
    ):
...
def verify_one(cache_root: Path, episode_id: str) -> list[str]:
    try:
        ...
        errors: list[str] = []
        for key in (
            "packet_version", "episode_id", "plan_sha256",
            "episode_artifact_sha256", "full_script_sha256",
