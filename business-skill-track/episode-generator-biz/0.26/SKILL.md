...
- 先建立不带分集编号的主干情绪运动，再写完整互动故事与路线级拓扑；从故事中已经发生的真实戏剧裂缝识别分支。路线去向冻结后才展开戏剧单位、确定节点数并生成正式逐节点情绪脊，不得先按节点预算画空分支树再让故事迁就结构。
...
1. 只读取`stage-two-input.json`和本阶段references，先建立不带分集编号、节点数量和拓扑形状的主干情绪运动，锁定压力、反差、释放和控制权变化。
...
4. 在完整故事内审计玩家可执行动作及其即时状态差，再依各路线的行动后果与情绪结算决定继续、汇合或结束；后续选择只放在因前序状态而真实出现该问题的路线，不要求所有路线经过同一组选择。不得先指定拓扑形状再补写情绪理由。
...
全部命令PASS后，完整故事、拓扑和全体梗概一起冻结，才允许写任何分集正文。PASS至少代表：阶段二生成包没有上游节点建议；完整故事可独立复述且路线级分支与结局成立；确定节点数来自全部路线的因果展开并与正式拓扑合同一致；拓扑编号连续、入口唯一、全部节点可达、无环、无断头、所有节点可达结局、选择目标有效、选择对应情绪拐点、汇合没有清除路线差异、结局数量正确；全体梗概完整覆盖故事且每集形成因果变化。脚本负责合同、指纹、数量、映射和证据完整性；Agent仍须判断每条路线的去向是否由行动后果与情绪结算产生，以及期待性结局、题材相称的失败力度和故事整体是否成立。
...
- `references/stage-three-release.md`
...
全体回执PASS后，不得直接进入阶段四。必须生成项目级剧本集合复检包，独立核验全集因果、核心目标、情绪轨迹、全部结局、题材期待及人话对白，并封存唯一阶段三放行凭证：

```bash
python3 scripts/stage_three_release.py packet <缓存目录> --output <临时项目级复检包.json>
python3 scripts/stage_three_release.py seal <缓存目录> <项目级复检结果.json>
python3 scripts/stage_three_release.py verify <缓存目录>
```

三个命令全部PASS才进入阶段四。逐集回执只证明各集在各自生成时有效；`stage-three-release.json`才是整套当前剧本允许被组装、投影或送往媒体生产的唯一凭证。完整分集材料与其中的完整剧本分别使用`episode_artifact_sha256`和`full_script_sha256`，不得混用。
...
组装器只能映射已验证内容，不得新增、改写或摘要剧本。组装器写入后生成私有完成凭证；业务JSON或任一绑定依赖变化时，凭证立即失效。

在任何NextPlay route提交、覆盖、故事板或视频任务开始前，必须对proposed route运行：

```bash
python3 scripts/stage_three_release.py verify-projection <缓存目录> <输出目录>/episode-business.json <proposed-route.json> <完成凭证.json>
```

该命令逐集强制比较阶段三完整剧本、九字段业务JSON和route中的`content.script.text`，并验证完成凭证仍绑定当前阶段三放行。命令FAIL时禁止提交route、禁止把旧媒体继续视为当前结果、禁止启动故事板或视频。正式route提交后再对读回文件重复同一命令。最终验证PASS后，一次性返回完整`分集列表`JSON；不得返回局部节点或内部材料。
...
- 全集当前正文另有一份项目级语义复检和有效的阶段三统一放行凭证；逐集回执、业务JSON和route使用明确区分的材料哈希与完整剧本哈希。
...
- proposed route和正式读回route都通过`verify-projection`，证明逐集完整剧本与阶段三放行及业务JSON逐字一致。
