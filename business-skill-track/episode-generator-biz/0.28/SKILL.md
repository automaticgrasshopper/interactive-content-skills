...
3. 计划PASS后运行`build_episode_writing_context.py`，把当前集已有冻结事实组装成私有写作材料。写正文时只使用该文件，不重新载入完整故事、情绪脊、无关路线或未来正文；该文件不创造新事实、不新增门禁，也不进入正式结果。
4. 把冻结梗概原样写入`episodes/<episode-id>.md`，再依据该私有材料完成九字段正文。不得临时改写梗概、拓扑或选择，也不得把梗概原句当成正文粘贴。
5. 单独提取台词，锁定事实、动作和结果，按`chinese-dialogue-craft.md`恢复必要的接话、反驳、迟疑和关系指向，再放回原位；这只是现有台词复写，不新增剧情、场面或验收层。
