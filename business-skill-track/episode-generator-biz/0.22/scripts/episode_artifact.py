...
def validate_change_scope(
    ...
    current_treatment = cache_root / "story-treatment.json"
    baseline_treatment = baseline_root / "story-treatment.json"
    if change_scope == "dialogue" and (
        not current_treatment.is_file()
        or not baseline_treatment.is_file()
        or current_treatment.read_bytes() != baseline_treatment.read_bytes()
    ):
        issues.append("对白修改期间完整故事发生变化")
    ...
    for node_id in node_ids:
        current_synopsis = cache_root / "episode-synopses" / f"{node_id}.json"
        baseline_synopsis = baseline_root / "episode-synopses" / f"{node_id}.json"
        if not current_synopsis.is_file() or not baseline_synopsis.is_file():
            issues.append(f"局部修改缺少当前或基线梗概：{node_id}")
        elif (
            node_id not in allowed_changed or change_scope == "dialogue"
        ) and current_synopsis.read_bytes() != baseline_synopsis.read_bytes():
            issues.append(f"未授权梗概发生变化：{node_id}")
