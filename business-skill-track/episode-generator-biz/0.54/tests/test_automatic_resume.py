...
class AutomaticResumeTests(unittest.TestCase):
    ...
    def test_episode_status_returns_exact_next_action(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            self.assertEqual(current["next_actions"][0]["action"], "WRITE_EPISODE")
            self.assertEqual(current["next_actions"][0]["execution_mode"], "current_task")
            self.assertTrue(current["next_actions"][0]["standard_input"].endswith("episode-writing-inputs/episode-001.txt"))
    ...
    def test_context_cut_resumes_live_lease_and_recovers_expired_lease(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            live["episodes"]["episode-001"]["leases"] = {
                "WRITE_EPISODE": {"lease": "live", "expires_at": run_state.iso(run_state.now() + timedelta(minutes=5))}
            }
            ...
            expired["episodes"]["episode-001"]["leases"] = {
                "WRITE_EPISODE": {"lease": "old", "expires_at": run_state.iso(run_state.now() - timedelta(minutes=5))}
            }
