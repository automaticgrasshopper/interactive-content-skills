#!/usr/bin/env python3
"""Atomic controller for adaptation, full writing, cold review and acceptance."""
...
VERSION = "nextplay.episode-atomic-run.v6"
...
CONTENT_RECOVERY = {
    "ADAPT": ("QUEUED", set()),
    "WRITE_EPISODE": ("ADAPTED", {"ADAPT"}),
    "VALIDATE_STRUCTURE": ("ADAPTED", {"ADAPT"}),
    "REVIEW_EPISODE": ("ADAPTED", {"ADAPT"}),
    "ACCEPT_EPISODE": ("QUALITY_REVIEWED", {"ADAPT", "WRITE_EPISODE", "VALIDATE_STRUCTURE", "REVIEW_EPISODE"}),
}
...
def fail(cache_root: Path, episode_id: str, action: str, lease: str, reason: str, failure_kind: str) -> None:
    ...
    if failure_kind == "content":
        ...
        state_name, keep = CONTENT_RECOVERY[action]
        episode["state"] = state_name
        episode["outputs"] = {
            key: value for key, value in (episode.get("outputs") or {}).items()
            if key in keep
        }
    else:
...
def technical_recover(cache_root: Path, episode_id: str, target: str, reason: str) -> None:
    allowed = {
        ...
        "REWRITE_EPISODE": ("ADAPTED", {"ADAPT"}),
        "RESEAL_STRUCTURE": ("SCREENPLAY_WRITTEN", {"ADAPT", "WRITE_EPISODE"}),
        "REREVIEW_EPISODE": ("STRUCTURE_VALIDATED", {"ADAPT", "WRITE_EPISODE", "VALIDATE_STRUCTURE"}),
        "REBIND_ACCEPTANCE": ("QUALITY_REVIEWED", {"ADAPT", "WRITE_EPISODE", "VALIDATE_STRUCTURE", "REVIEW_EPISODE"}),
    }
...
def status(cache_root: Path) -> dict[str, Any]:
    ...
    for episode_id in state["episode_order"]:
        episode = state["episodes"][episode_id]
        for action, count in (episode.get("content_failures") or {}).items():
            if int(count) >= MAX_CONTENT_ATTEMPTS:
                state["status"] = "STOPPED"
                state["reason_code"] = "CONTENT_REPAIR_EXHAUSTED"
                state["issues"] = [f"{episode_id}/{action}连续{count}次内容返修未通过"]
                state["next_actions"] = []
                return state
        for action, count in (episode.get("orchestration_failures") or {}).items():
            if int(count) >= MAX_ORCHESTRATION_ATTEMPTS:
                state["status"] = "STOPPED"
                state["reason_code"] = "ORCHESTRATION_RECOVERY_EXHAUSTED"
                state["issues"] = [f"{episode_id}/{action}连续{count}次技术恢复未成功"]
                state["next_actions"] = []
                return state
...
def main() -> int:
    ...
    try:
        ...
        print(f"FAIL: {error}"); return not_accepted(__file__)
    return 0
