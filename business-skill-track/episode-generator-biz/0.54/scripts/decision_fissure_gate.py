...
CONTRACT_VERSION = "nextplay.decision-fissure-audit.v5"
RECEIPT_VERSION = "nextplay.decision-fissure-review.v2"
ROOT_FIELDS = {"contract_version", "complete_story_sha256", "emotional_movement_sha256", "core_goal", "core_goal_source_proof", "fissures"}
FISSURE_FIELDS = {"fissure_id", "mainline_position_proof", "emotional_movement_ids", "decision_question", "actions", "disposition", "disposition_reason"}
ACTION_FIELDS = {
    "action_id", "action_text", "immediate_consequence", "emotional_direction",
    "emotional_cost", "unresolved_task_after", "core_goal_status", "route_ended",
    "ending_scope", "ending_reason", "goal_status_explanation",
}
...
def direction(value: Any, label: str) -> tuple[float, float, float]:
    if not isinstance(value, dict) or set(value) != POINT_FIELDS:
        raise ValueError(f"动作情绪方向字段错误：{label}")
    result: list[float] = []
    for field in ("valence", "arousal", "dominance"):
        item = value.get(field)
        if not isinstance(item, (int, float)) or isinstance(item, bool) or not -2 <= item <= 2:
            raise ValueError(f"动作情绪方向必须是-2到2的位移：{label}/{field}")
        result.append(float(item))
    if max(abs(item) for item in result) < 0.1:
        raise ValueError(f"动作没有真实情绪方向：{label}")
    return tuple(result)
...
def read_audit(cache_root: Path) -> tuple[Path, dict[str, Any]]:
    ...
    decomposition_errors, _, decomposition = decomposition_issues(cache_root)
    if decomposition_errors:
        raise ValueError("主线分解未通过：" + "；".join(decomposition_errors))
    source_by_segment = {
        str(item["segment_id"]): str(item["source_text"])
        for item in decomposition["segments"] if isinstance(item, dict)
    }
    movement_by_id = {
        str(item["movement_id"]): item for item in movement["movements"] if isinstance(item, dict)
    }
    ...
    if data.get("emotional_movement_sha256") != digest(movement):
        raise ValueError("决策裂缝审计未绑定当前主线情绪运动")
    ...
    for index, fissure in enumerate(fissures, 1):
        ...
        movement_ids = fissure.get("emotional_movement_ids")
        ...
        if not isinstance(movement_ids, list) or not movement_ids or any(
            not isinstance(item, str) or item not in movement_by_id for item in movement_ids
        ):
            raise ValueError(f"决策裂缝没有绑定有效情绪运动：{fissure_id}")
        movement_corpus = "\n".join(
            source_by_segment.get(segment_id, "")
            for item in movement_ids
            for segment_id in movement_by_id[item].get("segment_ids", [])
        )
        if proof not in movement_corpus:
            raise ValueError(f"决策裂缝位置未被所引情绪运动支持：{fissure_id}")
        ...
        for action in actions:
            ...
            vector = direction(action.get("emotional_direction"), f"{fissure_id}/{action_id}")
            emotional_cost = str(action.get("emotional_cost") or "").strip()
            unresolved_after = str(action.get("unresolved_task_after") or "").strip()
            ...
            if len(emotional_cost) < 6:
                raise ValueError(f"候选动作缺少可见情绪代价：{fissure_id}/{action_id}")
            if route_ended and unresolved_after != "已结算":
                raise ValueError(f"结束动作仍留有未结情绪任务：{fissure_id}/{action_id}")
            if not route_ended and (len(unresolved_after) < 6 or unresolved_after == "已结算"):
                raise ValueError(f"继续动作没有记录后续情绪任务：{fissure_id}/{action_id}")
            ...
            directions.add(vector); remaining_tasks.add(unresolved_after)
        if len(directions) != len(actions):
            raise ValueError(f"候选动作没有形成不同情绪方向：{fissure_id}")
        if len(remaining_tasks) != len(actions):
            raise ValueError(f"候选动作没有形成不同的结算或未结任务：{fissure_id}")
        direction_list = list(directions)
        if max(
            max(abs(a[index] - b[index]) for index in range(3))
            for offset, a in enumerate(direction_list)
            for b in direction_list[offset + 1:]
        ) < 0.2:
            raise ValueError(f"候选动作情绪分歧过弱：{fissure_id}")
...
def packet(cache_root: Path) -> dict[str, Any]:
    ...
    _, complete_story = read_complete_story(cache_root)
    _, movement = read_movement(cache_root)
    return {"packet_version": RECEIPT_VERSION, "audit_path": str(path), "audit_sha256": digest(audit), "complete_story_sha256": digest(complete_story), "emotional_movement_sha256": digest(movement), "required_checks": REQUIRED_CHECKS, "complete_story": complete_story, "mainline_emotional_movement": movement, "audit": audit}
...
def validate_review(packet_value: dict[str, Any], review: dict[str, Any]) -> list[str]:
    ...
    for key in ("audit_sha256", "complete_story_sha256", "emotional_movement_sha256"):
...
def main() -> int:
    ...
    except (OSError, ValueError, json.JSONDecodeError) as error: print(f"FAIL: {error}"); return not_accepted(__file__)
    return 0
