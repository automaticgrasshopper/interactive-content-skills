...
def validate_one(cache_root: Path, episode_id: str) -> list[str]:
    ...
    screenplay_path = cache_root / "screenplays" / f"{episode_id}.md"
    if not screenplay_path.is_file():
        issues.append(f"缺少完整编剧稿：{episode_id}")
    elif screenplay_path.read_text(encoding="utf-8").strip() != full_script:
        issues.append(f"正式正文不是完整编剧稿的逐字副本：{episode_id}")
    else:
        from validate_screenplay_quality import validate_screenplay_quality
        issues.extend(validate_screenplay_quality(cache_root, episode_id, full_script, catalog))
