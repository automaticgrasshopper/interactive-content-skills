...
把四项上游输入转成一份通过全部门禁的`分集列表`JSON。先完成全篇故事，再把故事结构化为拓扑和全体分集梗概，最后逐集写成剧本。各阶段依次放行，正式结果在最终验收通过后返回。

脚本检查合同、结构、证据、指纹和修改边界；Agent判断人物、因果、信息吞吐和台词是否成立。阶段放行同时以脚本结果和语义判断为依据。

## 架构约束

- 用户要求是创作约束，优先于上游建议、Skill默认规则和内部启发式。安全边界、输出合同或客观可实现性与用户要求冲突时，按相应边界处理；其余情况下保留用户指定内容。
- 用户要求之间无法同时成立时，暂停并使用AskUser确认；取得回答后继续。
- 初次生成与用户局部编辑是两个模式。初次生成从情绪运动推导路线与节点；已有分集结果上的新增、修改、删除和重连以用户操作后的当前节点关系为事实源，只处理授权节点及其相邻内容，不以情绪脊重新评价用户改动。
- 先建立不带分集编号的主干情绪运动，再写完整互动故事与路线级拓扑；从故事中已经发生的真实戏剧裂缝识别分支。分别计算“单条完整游玩路径需要的展开量”和“全图实际节点数”：前者保证各路线具有足够展开量，后者在全部路线展开后统计。路径体量不作为全图节点上限，也不预先按节点预算建立分支树。
- 全部项目共用本 Skill 定义的唯一因果组织与结局拓扑规则；时长只控制场面数量和展开容量，不触发结构规则切换。
- 题材作为稀疏的场面导演：从已冻结的`genre_tone`提取不超过3项类型承诺，安放到合适情绪窗口；不另建故事合同、节点义务或逐集打卡表。
- 完整故事、正式拓扑、全体分集梗概和逐集剧本是四层不同材料：故事决定发生什么，拓扑决定如何连接，梗概决定每集承载哪段变化，剧本负责把冻结梗概实际演出。上层内容确定后，下层只负责投影和展开。
- 一个`episode-xxx`对应一集有完整剧情的节点。有效选择只在来源分集的`互动节点`中封装一次。
- 选择后的不同事实分别演出，取得共同事实后带差异汇合；没有共同事实时继续分线。
- 初次生成中，冻结拓扑是剧本锚；明确解冻前，正文保持节点、边、选择、条件、状态效果和结局不变。用户局部编辑模式以操作后的当前图取代旧锚，只改授权范围。
- 写作按相邻因果推进，完整呈现触发、判断、欲望、行动、阻力、调整、即时后果和下一压力。
- 阶段一、二与最终组装由当前任务执行。阶段三具备隔离子任务能力时，只把已通过材料门禁的连续故事输入交给独立编剧，过程链、验收答案、旧稿问题和未来剧情不进入编剧输入；没有隔离能力时按相同文件边界顺序换档，门禁保持不变。
- `episode-001`带选择时，先完整演出第一集剧情，再显示选择。
- 默认主动完成全部阶段。输入缺失、用户要求冲突或需要取得新授权时使用AskUser。

## 运行模式

- **初次生成**：按阶段一至四执行，使用情绪运动、完整故事和因果展开生成正式拓扑。
- **用户局部编辑**：执行前完整读取`references/local-node-editing.md`。工程字段或route边只用于识别目标、直接前置和直接后续；节点引用视为不透明标识，不推断其命名含义。当前工作边界是一集，用户指定情节是内容约束；情绪脊、全局拓扑理由和未来结局不用于改写用户要求。
...
- 正式结果只有九字段`分集列表`JSON；其他文件和工程结构不属于交付结果。
- 情绪脊、完整故事、拓扑、梗概集合、缓存、复检材料、回执、完成凭证和错误详情都是私有运行材料，不进入正式结果。
- 长任务使用四条面向用户的阶段状态，每条在首次进入对应阶段时最多发送一次：冻结基础、情绪脊和完整故事期间发送`正在生成故事`；投影拓扑和冻结全体梗概期间发送`正在生成分支`；逐集写作与逐集放行期间发送`正在生成剧本`；最终检查与业务组装期间发送`正在最终验收`。除此之外不发送过程消息。
...
初次生成读取`游戏企划`、`角色描述`、`场景描述`、`道具描述`。缺少哪一项，只追问哪一项。`剧情节点总数建议`只在阶段一记录来源，不进入阶段二生成上下文，也不参与路线级拓扑或节点展开；用户明确要求固定为N集时，节点数作为用户约束进入阶段二。
...
阶段结果只以本轮命令的退出码和输出为准。命令、PASS和回执均由规定流程产生；门禁未通过时停留在当前阶段。
...
在创建冻结材料前判断四项输入是否齐全。脚本检查冻结材料的完整性和自洽性；Agent同时负责确认材料均有上游依据。
...
两个命令都PASS后冻结阶段一。随后运行`build_stage_two_input.py`生成不含上游节点建议的临时阶段二输入包。阶段二只使用该输入包，不直接重读`run-basis.json`、`user-request.md`或原始上游输入。出现`needs-user`、四项输入缺失、资产名非法或重名、结局数量矛盾、角色运行边界缺失时，停留在阶段一处理。
...
1. 只读取`stage-two-input.json`和本阶段references，先建立`unnumbered-emotional-movement.json`：不带分集编号、全图节点数量和拓扑形状，锁定压力、反差、释放、控制权变化、观众已知风险与候选戏剧裂缝；同时给出私有的单路径体量区间和互动探索区间。它们只约束一条完整游玩路径的展开量，不是正式节点数或对外字段。
   - 本次生成从该未编号情绪运动重新推导路线。旧`topology.md`、旧`emotional-spine.json`、旧分集梗概、旧分集正文和缓存中的节点、边、标题不属于阶段二输入。
...
5. 路线级拓扑通过后，把每条路线展开为不可再合并的可演戏剧单位。每个单位包含处境、目标、行动、阻力、调整和可见结果；完成全部路线后计算实际节点数。上游节点建议此时仍不可见，用户意图合同明确冻结固定集数时除外。
6. 编号全部戏剧单位并生成正式`topology.md`。在机器注释中写入`nextplay.episode-topology.v1`合同、实际节点数和`causal-expansion`来源；内容只投影完整故事中已经成立的事件、选择、汇合与结局。重大路线完成独立目标、阻力、状态变化和情绪轨迹后可以带差异汇合，不以是否改变可达结局集合判断其有效性。
7. 按正式拓扑生成`emotional-spine.json`，把此前主干情绪运动投影到每个实际节点；带选择节点必须对应真实拐点。把规范化情绪脊SHA-256写入拓扑机器注释，并校验完整故事、情绪脊和拓扑相互一致。
8. 按冻结拓扑一次生成全部`episode-synopses/<episode-id>.json`。每集梗概从完整故事中切出连续的因果变化，写清处境、目标、行动、阻力、调整、结果和下一入口；事实或风险清单不构成有效梗概。
   - 梗概进入正文时，展开为人物当场可见、可听、可执行的动作、阻力与结果；概念陈述、账目式记录、作者判断或信息摘要不替代场面。
...
命令中的`E`、`F`、`X`分别取自已通过阶段一门禁的结局总数、正式结局数和失败结局数，不使用模板常量。
...
- 当前题材标签命中`references/genre-direction.md`中的模块时读取该文件；否定标签不触发对应模块，未命中时不增加题材义务
- 当前集出现推动剧情的书面信息时，再读取`references/written-text-to-dialogue.md`
- 冷读时只读取脚本生成的复检包；`references/episode-quality-review.md`不进入提前判断

### 隔离工作集

阶段三开始时，按冻结拓扑把相邻因果节点划成不超过3集的工作批次。共同主线、不同分支、汇合段和结局段分别成批，互斥路线正文使用不同的活跃工作集。批次不改变拓扑和正式输出。每集内部再分为适配、编剧、写后验收三个互不共享多余材料的工作区。

- 同一时刻只写当前一集；当前集验收完成后再写下一集。
...
- 编剧工作区不接收过程链、情绪脊、完整故事、验收reference、证据格式、旧稿和审核意见。
- 已放行的更早正文保留在缓存中作为证据，并退出活跃工作集；定位连续性问题时可读取相关部分，其他情况下不纳入当前写作上下文。
- 每完成一批，确认批内各集`verify-one`仍为PASS，再切换到下一批。下一批只从冻结材料重建工作集，不纳入草稿、旧审稿意见、淘汰方案和无关路线材料；缓存证据保持不变。
- 批次中的每一集使用同一门禁。正文低于对应节点职能的软篇幅且缺少行动、阻力、调整或可见后果，或者出现事实罗列、条件式代写、人物口吻无法区分、用解释代替场面时，当前集不放行，只返修当前集。
...
4. 把Screenwriter放入隔离工作区，只给当前写作输入、`vimax-screenwriter.md`和台词reference，生成`screenplay-drafts/<episode-id>.md`。角色卡、过程链、预期出口、审核问题和旧稿不进入该工作区。
...
6. 把增强稿原样装入`episodes/<episode-id>.md`，冻结梗概与九字段保持原样。正文区与`enhanced-screenplays/<episode-id>.md`逐字一致；后续结构验收、场面展开复检和独立冷读均为只读，不产生第三版正文。增强稿完成后读取首次出场规则并提取证据；缺失时只登记问题，不在验收阶段补职位或改台词。
7. 运行当前集结构门禁。随后按冻结梗概独立建立`dramatization-plans/<episode-id>.json`并运行`dramatization_gate.py plan`；建立计划时不读取草稿、增强稿、写作输入或验收答案。
8. 生成场面展开复检包，逐过程核对增强稿。过程链只在此时作为写后判据出现，不返回给编剧或Enhancer，也不在本轮自动改稿。发现缺环时写入问题单并停止当前集放行。
...
- 单集梗概概括本集完整剧本实际演出的事件，不包含未演出的前史、未来结果、作者解释或其他分集内容；梗概使用可独立理解的自然语言。
- 单集梗概在阶段二整体冻结。阶段三将其演成正文，并保持原有剧情范围和分集边界。
- 场次使用`【场景名称·时段·内/外】`。
- 叙述按正常段落书写，不使用`△`、`出场：`、镜号、景别、机位、运镜或审稿标签。
- 对白使用`人物：台词`，不使用台词引号。
- 选择必须是玩家可执行的动作，并指向冻结拓扑中的真实目标集。
- 每集只解决当前主要问题并形成下一压力；不要一次塞入多轮设定、身份和反转。
- 当前集只演出冻结前置实际带来的路线事实，不用“若走某路线”“另一种情况下”“两种来路”“视玩家此前选择”等条件式文字同时代写互斥路线。汇合节点先取得拓扑规定的共同事实，再在共同处境中继续。
...
组装器只映射已验证内容，不新增、改写或摘要剧本。组装器写入后生成私有完成凭证；业务JSON或任一绑定依赖变化时，凭证立即失效。最终验证PASS后，一次性返回完整`分集列表`JSON；局部节点和内部材料不属于正式返回内容。

## 修改已有结果

- 用户在已有分集结果上新增、插入、删除、重连或实质改写节点时，优先进入`references/local-node-editing.md`定义的用户局部编辑模式；当前节点关系和用户修改是事实源，不退回阶段二重新评价该改动是否符合原情绪脊。
- 自动生成阶段发现完整故事或拓扑错误时，按最早失效阶段返修；用户已经明确执行的局部编辑按局部编辑规则处理。
...
- 只改对白：增加`--change-scope dialogue`；授权分集内的非台词内容、事实、动作、状态、选择和结尾保持不变。
- 创建、修改、重做和恢复都返回当前完整结果，不只返回变化片段。

详细参数和失效范围读取`references/run-basis-and-stage-gates.md`中的修改模式。局部修改后重新执行受影响分集的结构验收、冷读和回执重建。
