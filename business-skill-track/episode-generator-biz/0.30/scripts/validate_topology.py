...
def validate(
    ...
    if movement is not None and roots == ["episode-001"] and count == len(nodes):
        ...
        if not isinstance(scale, dict):
            ...
            path_min = scale.get("path_length_min")
            path_max = scale.get("path_length_max")
            ...
            for path in paths:
                ...
                if isinstance(path_min, int) and len(path) < path_min:
                    issues.append(f"正式结局路径短于单路径下限：{path[-1]}={len(path)}<{path_min}")
                if isinstance(path_max, int) and len(path) > path_max:
                    issues.append(f"正式结局路径长于单路径上限：{path[-1]}={len(path)}>{path_max}")
