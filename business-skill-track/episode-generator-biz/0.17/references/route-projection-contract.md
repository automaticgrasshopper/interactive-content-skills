...
本合同只负责把已通过全部业务门禁且持有当前完成凭证的九字段`分集列表`确定性投影为Nextplay路线，不改变创作、情绪脊、剧本、选择或结局。

## 唯一事实源

- 业务 JSON 是唯一事实源；投影器不得重新推导节点、选择、连线或结局。
- 业务 JSON 只能由业务组装器产生。投影器必须从当前缓存重跑完整项目门禁、重建业务对象并与输入 JSON 逐值相等，同时验证私有完成凭证；只通过字段 Schema 或路线结构校验不足以获得投影资格。
- `route.data.nodes`同时含剧情节点与选择节点，并按网页实际显示顺序统一使用连续的`episode-xxx`作为`node_ref`和`episode_ref`。业务分集原编号保存在`metadata.business_episode_ref`；路线层派生的选择编号保存在`metadata.choice_ref`与`interaction.interaction_ref`。
- 业务 JSON 中的选择只封装在来源分集的`互动节点`一次；路线投影层按分集顺序据此派生唯一连续`choice-xxx`，不增加业务分集对象。
...
- 有选择时，剧情节点用一条`default`边连接紧随其后的唯一独立`node_type=choice`选择节点；该节点仍使用已冻结的连续`episode_ref`，固定`interaction.has_interaction=true`，填写路线层派生的选择编号、业务问题和选择边引用，每个选项从该节点生成一条`choice`边。
...
```text
python3 scripts/build_route_projection.py BUSINESS_JSON BASE_ROUTE_JSON ASSETS_JSON PROPOSED_ROUTE_JSON --outline-revision N --assets-revision A --asset-catalog ASSET_CATALOG --cache-root CACHE_ROOT --public-dir PUBLIC_DIR --completion-receipt COMPLETION_RECEIPT --expected-endings E --expected-formal F --expected-failure X
