...
def validate(business: dict[str, Any], route: dict[str, Any]) -> list[str]:
    ...
    expected_choice_ids = list(choice_ids_by_episode.values())
    ...
    for episode in episodes:
        ...
        episode_route_refs[episode["分集编号"]] = route_ref
        expected_node_order.append(route_ref)
        visible_index += 1
        if episode["互动节点"]["是否有选择问题"] is True:
            choice_id = choice_ids_by_episode[episode["分集编号"]]
            choice_route_refs[choice_id] = f"episode-{visible_index:03d}"
            expected_node_order.append(f"episode-{visible_index:03d}")
            visible_index += 1
    ...
    if "settings" in route:
        issues.append("route不得包含工程合同未声明的settings")
    ...
    for episode in episodes:
        ...
        route_episode_id = episode_route_refs[episode_id]
        video = node_by_id.get(route_episode_id)
        if not isinstance(video, dict):
            continue
        interaction_record = episode["互动节点"]
        has_choice = interaction_record["是否有选择问题"]
        if not isinstance(has_choice, bool):
            issues.append(f"{episode_id}/是否有选择问题不是boolean")
            continue
        if interaction_record["是否为分支节点"] is not has_choice:
            issues.append(f"{episode_id}/分支与选择标记不一致")
        if video.get("node_type") != "video" or video.get("episode_ref") != route_episode_id:
            issues.append(f"{episode_id}/必须投影为保留网页编号的video剧情节点")
        if video.get("metadata", {}).get("business_episode_ref") != episode_id:
            issues.append(f"{episode_id}/缺少业务互动节点映射")
        ...
        if episode["是否结局"]:
            ending_ids.add(route_episode_id)
        ...
        video_edges = outgoing.get(route_episode_id, [])
        ...
        if has_choice:
            ...
            choice_route_id = choice_route_refs[choice_id]
            choice_node = node_by_id.get(choice_route_id)
            ...
            if choice_node.get("metadata", {}).get("logical_node_type") != "choice":
                issues.append(f"{choice_id}/缺少独立逻辑选择节点标记")
            if choice_node.get("episode_ref") != choice_route_id:
                issues.append(f"{choice_id}/网页episode_ref必须保留连续原编号")
            if choice_node.get("metadata", {}).get("choice_ref") != choice_id or choice_node.get("metadata", {}).get("source_business_episode_ref") != episode_id:
                issues.append(f"{choice_id}/选择节点业务映射错误")
            choice_script = choice_node.get("content", {}).get("script", {}).get("text")
            if not isinstance(choice_script, str) or not choice_script.strip():
                issues.append(f"{choice_id}/必须保留网页展示所需的content.script.text")
            ...
            if not isinstance(choice_interaction, dict):
                ...
                continue
            option_edges = outgoing.get(choice_route_id, [])
            ...
            if choice_interaction.get("question") != interaction_record["选择问题"]:
                issues.append(f"{choice_id}/选择问题投影错误")
            if not isinstance(choice_interaction.get("after_plot_beat"), str) or not choice_interaction.get("after_plot_beat").strip():
                issues.append(f"{choice_id}/after_plot_beat不得为空")
            if choice_interaction.get("option_edge_refs") != [edge.get("edge_ref") for edge in option_edges]:
                issues.append(f"{choice_id}/选择边引用错误")
            if len(video_edges) != 1 or video_edges[0].get("edge_type") != "default" or video_edges[0].get("target_node_ref") != choice_route_id:
                issues.append(f"{episode_id}/剧情播完后必须仅默认连接{choice_id}")
            options = interaction_record["选项列表"]
            if [edge.get("target_node_ref") for edge in option_edges] != [episode_route_refs[target] for target in expected_targets]:
            ...
            if [edge.get("target_node_ref") for edge in video_edges] != [episode_route_refs[target] for target in expected_targets]:
...
def main() -> int:
    ...
    choice_count = sum(episode["互动节点"]["是否有选择问题"] for episode in business["分集列表"])
    print(
