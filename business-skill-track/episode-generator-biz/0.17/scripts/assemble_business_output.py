#!/usr/bin/env python3
"""Build the business JSON only after the complete v0.1.53 gates pass."""
...
from completion_gate import make_receipt, validate_public_bundle
from episode_quality_gate import verify_project
from validate_and_assemble_scripts import (
    asset_values,
    atomic_write,
    parse_options,
    predecessors,
...
BUSINESS_SKILL_VERSION = "episode-generator-biz/0.17"
...
def build_business_output(
    ...
    issues.extend(validate_public_bundle(cache_root, public_dir, nodes))
    data = {
        "分集列表": episodes,
    }
    if not issues:
...
def main() -> int:
    ...
    try:
        ...
        if public_dir == cache_root or public_dir in cache_root.parents:
            raise ValueError("缓存目录必须位于公开目录之外")
        if completion_receipt.parent == public_dir or public_dir in completion_receipt.parents:
            raise ValueError("完成凭证必须保存在公开目录之外")
        asset_catalog_path = (
    ...
    print(
        ...
        "v0.1.53 gates, public bundle, business output, and completion receipt verified"
    )
    return 0
