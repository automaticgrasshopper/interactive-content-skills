...
1. 只读取`stage-two-input.json`和本阶段references，先建立`unnumbered-emotional-movement.json`：不带分集编号、全图节点数量和拓扑形状，锁定压力、反差、释放、控制权变化、观众已知风险与候选戏剧裂缝；同时固定所有结局路径的合法下限为2个剧情节点，并从体量与故事承载力给出正式结局路线目标区间和互动探索区间。合法下限允许首次选择之后用独立剧情节点演出因果充分的小结局；正式路线目标只保护主线展开量。它们都不是全图节点数或对外字段。
...
3. 只读取冻结主线、未编号情绪运动和阶段二输入，生成`story-treatment.json`。保持主线逐事件不变，从主线的真实决策裂缝为非主线动作生长支线，写清即时后果、持续差异、汇合条件与其他结局。支线可短于、等于或长于主线，可二级开扇或完全分线；只服从自己的因果、情绪运动与上游单次游戏时长上限。
...
全部命令PASS后，线性主线、支线故事图、拓扑、路线时长和全体梗概一起冻结，才允许写任何分集正文。PASS至少代表：主线是一篇无分支可能性语言的完整故事；支线没有改写主线，可短于、等于或长于主线；每次选择后先判断是否已经形成结局；所有入口到结局路线均不超过上游单次游戏时长；确定节点数来自全部路线的因果展开；拓扑编号连续、入口唯一、全部节点可达、无环、无断头、选择对应真实拐点且汇合保留差异；全体梗概完整覆盖主线与支线并且每集形成因果变化。
