...
2. `story-treatment.json`在不改写主线的前提下记录支线生长、选择后果与全部结局。
3. `topology.md`把故事图拆成一节点一集；`route-duration.json`用分钟校验全部路线。
4. `episode-synopses/*.json`把每个节点冻结为可写梗概。
5. 分集正文只改编对应梗概，不反向改故事或拓扑。
...
`story-treatment.json`使用`nextplay.episode-story-treatment.v2`：

```json
{
  "contract_version": "nextplay.episode-story-treatment.v2",
  "title": "故事标题",
