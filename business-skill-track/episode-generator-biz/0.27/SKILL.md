...
阶段三的第一交付对象是观众，不是门禁表。冻结梗概和`Process Chain`只规定必须演出的因果，不规定正文句序；不得把计划中的`触发依据—动作—核验—结果`逐项换词后排成短段。先在人物关系和现场压力中写成连续场面，再从成稿反向抽取证据。为凑证据而写出的句子即使能被脚本引用，也视为未完成剧本。

正文优先级固定为：观众一遍看懂正在发生什么；人物像本人一样说话和应对；行动遭遇阻力并引发调整；最后才是证据与合同完整。后项不得牺牲前项。
...
- 普通多角色场必须让对白、动作和反应按现场先后交织；不得先用连续动作段把事件全部办完，再让角色在场尾轮流复述结果、主题或选项。纯追逐、无交流潜行、蒙太奇和独角行动除外，但必须由当前梗概与场景职能明确支持。
- 一个普通节点只保留一个主要冲突。若冻结梗概要求在同一集依次完成三项以上彼此可独立成立的重大任务，阶段三不得压缩执行，必须退回阶段二重新切分戏剧单位。
...
3. 把冻结梗概原样写入`episodes/<episode-id>.md`，再依据已通过的场面展开计划完成九字段正文。写作时把过程链重新组织成连续场面：人物先感知和回应，再行动、受阻、调整；对白出现在它实际改变行动的位置。不得临时改写梗概、拓扑或选择，不得把梗概原句当成正文粘贴，也不得按过程编号逐段转述。
4. 先做一次“摘掉台词仍像梗概吗”的场面检查：若叙述只是连续报出动作和结果，或全部台词集中在事件完成后，退回第3步重写场面。通过后才单独提取台词，执行轻量台词复写并放回原位。
5. 首次出场角色出现时，立即把身份、关系、当前必要性三条逐字证据写入`character-introductions.json`。
6. 运行当前集结构门禁。
7. 结构门禁PASS后生成场面展开复检包。证据只能从已经完成的自然场面中抽取，不得为了匹配过程字段返向添加说明句。每个过程分别引用行动、阻力或核验、结果三处按序且不重复的正文证据；事件结束状态晚于全部过程。任何重大结果没有过程链、过程只能靠一句旁白证明、或证据按计划顺序成串出现却没有人物回应与现场阻力，均视为未展开。场面展开回执PASS前不得进入独立冷读。
...
- 场面不是“每段完成一项任务”的流水账。人物行动以后，至少让现场、对手或同伴作出能够改变下一步的反应；相邻段落之间必须存在回应关系，而不只是时间先后。
- 禁止工程说明语、审稿语、拓扑语和作者结论进入人物台词或叙述，例如“真实行动”“路线差异”“完成选择”“承重点”“推进结果”。题材专名可以保留，但首次出现时必须由动作或白话解释让观众听懂。
