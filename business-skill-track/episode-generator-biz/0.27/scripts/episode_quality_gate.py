...
def scene_style_issues(script: str) -> list[str]:
    """Catch high-confidence synopsis cadence before semantic review can self-PASS."""
    errors: list[str] = []
    for pattern in META_SCRIPT_PATTERNS:
        match = re.search(pattern, script)
        if match:
            errors.append(f"完整剧本出现工程、拓扑或作者摘要措辞：{match.group(0)}")
    ...
    for chunk in chunks:
        ...
        if not lines:
            continue
        scene_name = lines[0] if lines[0].startswith("【") else "未命名场景"
        body = lines[1:] if lines[0].startswith("【") else lines
        kinds = ["dialogue" if re.match(r"^[^：\n]{1,20}：", line) else "narration" for line in body]
        dialogue_count = kinds.count("dialogue")
        if dialogue_count < 4:
            continue
        first_dialogue = kinds.index("dialogue")
        narration_before = sum(kind == "narration" for kind in kinds[:first_dialogue])
        narration_after = sum(kind == "narration" for kind in kinds[first_dialogue + 1:])
        if narration_before >= 6 and narration_after == 0:
            errors.append(
                f"{scene_name}先连续完成大量动作，再在场尾集中追加对白；必须把话茬放回影响行动的位置"
            )
...
def validate_review(packet: dict[str, Any], review: dict[str, Any]) -> list[str]:
    ...
    for check in REQUIRED_CHECKS:
        ...
        if check == PLAIN_LANGUAGE_CHECK:
            ...
            errors.append(f"复检证据不在完整剧本中：{check}")
    if review.get("episode_artifact_sha256") != packet["episode_artifact_sha256"]:
        errors.append("复检九字段分集材料指纹与当前复检包不一致")
    if review.get("full_script_sha256") != packet["full_script_sha256"]:
        errors.append("复检完整剧本正文指纹与当前复检包不一致")
...
def _raw_verify_receipt(cache_root: Path, episode_id: str) -> list[str]:
    ...
    errors: list[str] = []
    for key in (
        "packet_version", "episode_id", "episode_artifact_sha256",
        "full_script_sha256", "reference_name",
        "reference_sha256", "user_intent_source_sha256",
        "user_intent_contract_sha256", "synopsis_set_sha256", "required_checks",
    ):
...
def build_packet_unchecked(cache_root: Path, episode_id: str) -> dict[str, Any]:
    ...
    style_errors = scene_style_issues(script)
    if style_errors:
    ...
    return {
        "packet_version": "episode-quality-gate-v5",
        "skill_version": version,
        "episode_id": episode_id,
        "script_path": str(path),
        "episode_artifact_sha256": sha256(artifact),
        "full_script_sha256": sha256(script),
...
def seal(cache_root: Path, episode_id: str, review_path: Path) -> Path:
    ...
    receipt = {
        ...
        "episode_id": episode_id,
        "episode_artifact_sha256": packet["episode_artifact_sha256"],
        "full_script_sha256": packet["full_script_sha256"],
