...
9. 生成梗概前完整读取`references/story-to-episode-synopsis.md`，按“当前节点共同事实／选项边动作／目标节点即时后果”投影冻结故事。按冻结拓扑一次生成全部`episode-synopses/<episode-id>.json`。每集梗概必须从冻结主线或支线中切出连续的因果变化，写清处境、目标、行动、阻力、调整、结果和下一入口；不得只列事实或风险。
