...
独立小结局不等于死亡或灾难。动作使人物放弃核心目标、主动离开主要场域、错过不可恢复的时机、转向与本故事无关的生活，或让关系与资源状态不再提供合理回归动力时，当前故事已经进行不下去，必须判为`route_ended=true`、`ending_scope=minor`。不得为了让它回到主线而强行巧遇、回心转意或取消已发生的偏离。每次初次生成至少采用2个这样的独立小结局动作；它们不占主要结局预算，且必须来自不同动作，不能用同一结果改名凑数。
