...
7. 先完成全部节点、分支、汇合和终点，但暂不编号。结构冻结后再按程序显示顺序统一编号：从入口向右逐层推进，同一层按父节点的选项顺序从上到下排列；一个分支必须从起点到终点保持自身上下位置，选择节点也按所在层编号。禁止先编号主线、再把支线编号补到全图末尾。随后生成正式`topology.md`与`mainline-path.json`，把每个主线切片一一映射到最终`episode-id`。主要终点的互动类型只能写`主要正式结局`或`主要失败结局`；动态终点写`独立小结局`。
