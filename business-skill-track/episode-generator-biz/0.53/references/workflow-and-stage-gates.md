...
四个阶段均由当前任务串行执行。每个安全提交点运行`workflow_state.py CACHE_ROOT`并立即执行唯一`next_actions`；上下文切分后也从该状态恢复，不依赖对话记忆或用户确认。所有命令的处理状态、退出码、恢复标记与询问权限统一遵守`execution-result-protocol.md`。
