...
- 写作输入只继承冻结梗概、接缝事实、互动边界、人物资产和用户硬约束；不得携带情绪脊、VAD数值、movement编号、拓扑推理或验收过程。
...
返修必须完整重做当前动作，不对已提交正文做局部补丁。内容门禁失败属于内部运行态；当前任务自行定位、返修、复验并继续。相邻接缝的最小必要修复属于本次编辑的隐含授权；只有用户独占的必要输入缺失、两条用户硬约束冲突，或刷新后仍存在多个当前目标时才对外询问。
