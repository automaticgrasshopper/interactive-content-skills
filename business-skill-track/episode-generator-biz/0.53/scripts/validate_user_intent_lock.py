#!/usr/bin/env python3
"""Validate the minimal user-intent lock used by episode-generator 0.53."""
...
CONFLICT_FIELDS = {"conflict_id", "constraint_ids", "question", "options"}
    ...
    if isinstance(conflicts, list):
        for index, conflict in enumerate(conflicts, 1):
            if not isinstance(conflict, dict) or set(conflict) != CONFLICT_FIELDS:
                issues.append(f"用户冲突第{index}项字段错误")
                continue
            conflict_ids = conflict.get("constraint_ids")
            if (
                not isinstance(conflict_ids, list)
                or len(conflict_ids) < 2
                or len(conflict_ids) != len(set(conflict_ids))
                or any(item not in ids for item in conflict_ids)
            ):
                issues.append(f"用户冲突必须引用至少两条当前有效硬约束：{index}")
            if len(str(conflict.get("question") or "").strip()) < 4:
                issues.append(f"用户冲突缺少中性问题：{index}")
            ...
            if (
                ...
                issues.append(f"用户冲突必须提供至少两个互斥选项：{index}")
        if contract.get("resolution_status") == "needs-user" and not conflicts:
            issues.append("needs-user必须包含可追溯的用户硬约束冲突")
