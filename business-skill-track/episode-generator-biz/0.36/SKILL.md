...
3. 在任何支线故事或拓扑之前生成`decision-fissure-audit.json`。逐字冻结原始`core_goal`并扫描全部真实决定裂缝；明确交易必须同时审计真实服从与拒绝。原始目标不再继续时`route_ended`必须为真，并标记`ending_scope`：只有正好兑现上游预定主要结局方向时写`main`，其余提前终止一律写`minor`；继续路线写`none`。含结束动作的裂缝必须采用。
4. 只读取冻结主线、通过的裂缝审计、未编号情绪运动和阶段二输入，生成`story-treatment.json`。`endings`只含上游预算内的主要结局；`minor_endings`逐一承接所有`ending_scope=minor`动作，动态增加且不占主要结局数量。每个独立小结局必须有唯一`source_action_id`。
...
5. 路线级故事图通过后，把主线和每条支线分别展开为不可再合并的可演戏剧单位。每个单位必须完成处境、目标、行动、阻力、调整和可见结果；完成全部路线后计算实际节点数。
6. 编号全部戏剧单位并生成正式`topology.md`。主要终点的互动类型只能写`主要正式结局`或`主要失败结局`；动态终点写`独立小结局`。每个提前终止动作必须直接指向自己的独立小结局节点；该节点无后继、不可汇合、不可继续主线。不同动作不得共享小结局。
...
9. 按冻结拓扑一次生成全部`episode-synopses/<episode-id>.json`。每集梗概必须从冻结主线或支线中切出连续的因果变化，写清处境、目标、行动、阻力、调整、结果和下一入口；不得只列事实或风险。
...
全部命令PASS后，线性主线、决策裂缝审计、支线故事图、拓扑、路线时长和全体梗概一起冻结，才允许写任何分集正文。PASS至少代表：主线是一篇无分支可能性语言的完整故事；全篇真实决定裂缝已扫描，任何因果充分的提前结局动作都被采用，较弱汇合选择没有替代它；支线没有改写主线，可短于、等于或长于主线；每次选择后先判断是否已经形成结局；所有入口到结局路线均不超过上游单次游戏时长；确定节点数来自全部路线的因果展开；拓扑编号连续、入口唯一、全部节点可达、无环、无断头；全体梗概完整覆盖主线与支线并且每集形成因果变化。
...
5. 退出编剧工作区后，运行`build_enhancer_input.py`，把当前草稿、停止边界、`vimax-script-enhancer.md`与`chinese-dialogue-craft.md`逐字组装为`enhancer-inputs/<episode-id>.txt`。隔离Enhancer只读取该文件，一次性生成`enhanced-screenplays/<episode-id>.md`。Enhancer可以完整改写场面表达、全部对白、话轮、停顿、反应和不改变因果的动作组织，不要求保留草稿原句；禁止改变冻结剧情事实、人物行动结果、选择、结局或停止边界。
...
python3 scripts/build_enhancer_input.py <缓存目录>/screenplay-drafts/<分集编号>.md --boundary "<停止边界>" --enhancer-reference references/vimax-script-enhancer.md --dialogue-reference references/chinese-dialogue-craft.md --output <缓存目录>/enhancer-inputs/<分集编号>.txt
