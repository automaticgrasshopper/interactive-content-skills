...
- 未取得`DELIVERABLE_ACCEPTED`时，必须从最早未通过的门禁继续执行，不得以最终答复结束；工作量和流程长度不构成中止条件。仅在缺少必需输入、用户要求冲突、需要新授权或工具持续故障时，才允许暂停并说明阻塞。
...
- 每完成一批，确认增强稿已经逐字装入正式正文并取得当前单集验收回执，再切换到下一批。切换后不再把草稿、淘汰方案和无关路线材料纳入活跃工作集。
...
7. 先运行当前集确定性结构门禁，检查九字段、拓扑关系、资产白名单、增强稿逐字一致、角色首次出场与停止边界绑定。
8. 退出写作工作区。按冻结梗概独立建立`dramatization-plans/<episode-id>.json`，生成场面复检包并只读检查当前增强稿；随后生成独立冷读包，由不读取过程链的冷读者检查同一份增强稿。两个检查者都只能写问题和位置，不能改稿、补句或生成替代正文。
9. 两项检查均无问题时，分别封存`dramatization-receipts/<episode-id>.json`与`episode-quality-receipts/<episode-id>.json`。任一检查有问题时，两项都输出绑定当前正文指纹的`review-findings/<episode-id>.<source>.json`，其中无问题的一项写`PASS`和空问题；再由`review_repair_gate.py merge`合并、去重并冻结行号范围；不得把过程链、证明格式或逐项通过要求写入问题单。
10. 局部修复仍属于Enhancer阶段。运行`review_repair_gate.py input`生成唯一修复输入，隔离Enhancer只改合并问题单命中的行或相邻接缝；未授权行逐字不变。修复不得改变冻结事实、行动结果、选择、结局或停止边界。若问题覆盖整集、连续故事材料错误或必须整集重写，局部修复门禁必须拒绝；重新写作须回到当前集Screenwriter并再次经过完整Enhancer，不得由复检者直接重写。
11. 修复后增强稿原位更新，正式正文再次逐字采用它；旧结构、场面和冷读回执全部失效。重新执行第7至10步，直到两项复检均PASS。不得为了通过检查把问题单逐项改成压缩旁白、举证句或人物轮流汇报。
12. 运行`episode_acceptance.py`。只有适配源、连续故事材料、写作输入、原稿、原稿回执、Enhancer输入、增强稿、正式正文、结构验收、场面复检回执和冷读回执同时存在且指纹一致，才生成`episode-acceptance-receipts/<episode-id>.json`并开始下一集。
...
python3 scripts/dramatization_gate.py packet <缓存目录> <分集编号> --output <临时场面复检包.json>
...
python3 scripts/episode_quality_gate.py packet <缓存目录> <分集编号> --output <临时冷读包.json>
...
python3 scripts/review_repair_gate.py merge <缓存目录> <分集编号> <缓存目录>/review-findings/<分集编号>.dramatization.json <缓存目录>/review-findings/<分集编号>.cold-read.json --output <缓存目录>/merged-review-findings/<分集编号>.json
...
`review_repair_gate.py`三条命令只在至少一项复检失败时运行；两项直接PASS时不得制造空修复。每集通过必须同时满足：连续故事材料绑定当前冻结梗概；编剧输入没有清单、证据或内部结构；增强稿正文与正式候选逐字一致；九字段结构、拓扑、资产、首次出场和停止边界一致；场面复检与独立冷读回执均绑定当前正文；单集验收回执完整。写完全部分集后，运行两项全体复检再进入阶段四。
