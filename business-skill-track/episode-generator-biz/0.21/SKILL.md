...
5. 只把完整故事中已经成立的事件和选择投影为`topology.md`，明确唯一入口、节点、后继、选择问题、选项、即时结果、汇合和结局；不得在拓扑阶段发明新剧情。
...
全部命令PASS后，完整故事、拓扑和全体梗概一起冻结，才允许写任何分集正文。PASS至少代表：完整故事可独立复述且分支与结局成立；拓扑编号连续、入口唯一、全部节点可达、无环、无断头、所有节点可达结局、选择目标有效、选择对应情绪拐点、结局数量正确；全体梗概完整覆盖故事且每集形成因果变化。脚本负责合同、指纹、数量、映射和证据完整性；Agent仍须对故事是否真正成立作冷静语义判断。
...
1. 只携带已冻结的当前集梗概、当前节点、直接前置结果、仍有效的状态差异、相关人物与资产、世界规则和直接后续入口。完整故事只用于保持全局方向，不得借它越过当前梗概塞入后续事件。
2. 把冻结梗概原样写入`episodes/<episode-id>.md`，再完成九字段正文。不得临时改写梗概、拓扑或选择。
3. 单独提取台词，执行轻量台词复写并放回原位。
4. 首次出场角色出现时，立即把身份、关系、当前必要性三条逐字证据写入`character-introductions.json`。
5. 运行当前集结构门禁。
6. 当前集结构门禁PASS后，冷读判断只允许引用脚本生成的当前集复检包和独立复检reference；不得引用写作笔记、作者计划、预期答案或后续剧情，随后验证理解门、梗概与正文一致及其余质量项。
7. 有问题时只返修当前集正文；若发现冻结梗概本身无法成立，停止阶段三并退回阶段二，重建受影响梗概集合及其回执，不能在正文中暗改梗概。
8. 正文变化后重新生成冷读包。问题清空并封存回执后，当前集`verify-one` PASS，才开始下一集正文。
...
每集通过必须同时满足：全体梗概集合已有当前有效回执；九字段结构正确；前后节点、选择、结局和资产与冻结材料一致；正文非空且场次格式正确；正文完整演出冻结梗概中的行动、阻力、调整与结果；理解门四项有互不替代的正文证据；全部冷读项覆盖；问题为空；当前正文、reference、冻结梗概集合和用户意图指纹与回执一致。
