...
VERSION = "nextplay.episode-stage-two-acceptance.v2"
...
def content_leaves(cache_root: Path) -> dict[str, Any]:
    """Stable semantic dependencies. Receipt formatting and review metadata are excluded."""
    global_names = (
        "run-basis.json", "stage-two-input.json", "mainline-story.json",
        "mainline-decomposition.json", "mainline-path.json", "story-treatment.json",
        "topology.md", "route-duration.json", "emotional-spine.json",
        "user-request.md", "user-intent-lock.json",
    )
    ...
    return {
        ...
        "content_sha256": hashlib.sha256(json.dumps(
            {"global": global_hashes, "episodes": episodes},
...
def load_verified(cache_root: Path) -> dict[str, Any]:
    ...
    if receipt.get("content_leaves") != content_leaves(cache_root):
        raise ValueError("阶段二内容叶绑定已失效")
    return receipt
...
def verify_binding(cache_root: Path) -> list[str]:
    ...
    try:
        ...
        if state.get("stage_two_content_sha256") != current:
            raise ValueError("运行状态未绑定当前阶段二内容")
    except (OSError, ValueError, json.JSONDecodeError) as error:
        return [str(error)]
    return []
