...
每集先建立`episode-synopses/<episode-id>.json`，梗概理解回执写入`synopsis-reviews/<episode-id>.json`；理解门通过后才允许建立`episodes/<episode-id>.md`，完整冷读回执写入`quality-reviews/<episode-id>.json`。不得先批量写完再补回执。
...
标题、前置和后续必须与冻结拓扑一致。梗概理解门通过前，当前集完整正文不得存在；当前集未完成`verify-one`前，不得存在其他未放行梗概或正文。

单集固定顺序：

```text
梗概 → synopsis-packet → 独立理解 → synopsis-seal → synopsis-verify-one
→ 完整写作 → 台词复写 → 结构校验 → packet → 独立冷读
→ 返修循环 → seal → verify-one
```

`packet`只生成冷读输入，不代表通过。`seal`只接受问题为空、理解门与全部复检证据齐全、指纹和用户意图绑定一致的复检结果。正文变化后必须重新packet、冷读和seal。

所有理解门与复检证据必须逐字存在于对应梗概或正文。当前集直接前置分集的完整回执未通过、发现其他未放行梗概或正文、梗概与正文中的梗概/冲突不一致时，脚本必须拒绝生成完整冷读包。
...
完成凭证使用`nextplay.episode-completion.v3`，绑定：
