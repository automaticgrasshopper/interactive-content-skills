...
创建完成后立即运行`python3 scripts/validate_mainline_projection.py CACHE_ROOT --decomposition-only`。只有该命令输出`PASS`，才能创建决策裂缝审计或任何支线材料。
...
拆分后重新运行同一分解门禁；新门禁通过前，已有决策裂缝回执自动失效。不得改变原文以迁就选择形式。
...
正式拓扑必须经过强制两版生成。第一版保存为`topology-draft-1.md`后立即废弃，不得成为任何下游材料的事实源。第二版必须重新从冻结故事材料推导，并且图形指纹与第一版不同；改名、换编号、改选项文字或仅换节点文案都不构成新形状。只有`topology.md`这个第二版可进入时长、情绪脊、梗概和正文。
