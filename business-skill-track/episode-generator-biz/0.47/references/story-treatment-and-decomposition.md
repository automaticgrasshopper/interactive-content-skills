...
`decision-fissure-audit.json`使用`nextplay.decision-fissure-audit.v2`。它只能在`mainline-decomposition.json`通过分解门禁后创建；决策裂缝复检回执绑定当前主线分解哈希，分解变化后必须重新审计。根对象必须逐字复制`stage-two-input.json`的`core_goal`并给出主线证据。沿已通过的主线切片从开场扫描到结局，每个候选记录主线逐字位置、决定问题、至少两个可执行且互斥动作、即时后果、核心目标状态、路线是否结束、`ending_scope`、对照原始核心目标的解释和理由。
