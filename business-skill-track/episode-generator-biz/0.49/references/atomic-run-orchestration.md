...
- 每次动作前先运行`run_state.py status`，只执行返回的`next_episode_id`和`next_actions`，不得按说明文字自行推断状态。
...
- 当前动作安全落盘后仍有后续工作时保存`IN_PROGRESS`。余量未知不等于失败；已验收分集不得重写。
...
- Screenwriter只读取通过门禁的`episode-writing-inputs/<episode-id>.txt`、`vimax-screenwriter.md`和`chinese-dialogue-craft.md`。
