...
3. 在任何支线故事或拓扑之前生成`decision-fissure-audit.json`。从主线开场扫描到结局，登记全部真实决定裂缝、当时可执行且互斥的动作、各动作即时后果，以及后果发生后核心目标是继续、完成、失败、放弃还是不可达。目标不再继续时`route_ended`必须为真；含任一因果充分结束动作的裂缝必须采用，不能用同位置较弱的快速汇合选择替代。独立复检并封存`decision-fissure-review.json`。
