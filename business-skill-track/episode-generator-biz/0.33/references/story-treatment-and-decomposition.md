...
`decision-fissure-audit.json`使用`nextplay.decision-fissure-audit.v1`。从冻结主线开场扫描到结局，每个候选记录主线逐字位置、决定问题、至少两个可执行且互斥动作、即时后果、核心目标状态、路线是否结束和理由。`core_goal_status`只能是`continuing`、`achieved`、`failed`、`abandoned`或`unreachable`；除`continuing`外都必须令`route_ended=true`。

候选只能标记`adopt`或`reject-causally-insufficient`。只要任一动作在即时后果后已经让核心目标完成、失败、放弃或不可达，该裂缝必须`adopt`，不得被同位置更弱的快速汇合选择替代。独立复检必须确认全篇扫描、动作可执行性、即时后果、目标状态、全部结局机会和替代风险，之后才能生成支线故事。
