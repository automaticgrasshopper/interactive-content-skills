...
RECEIPT_VERSION = "nextplay.episode-story-treatment-review.v5"
TREATMENT_NAME = "story-treatment.json"
RECEIPT_NAME = "story-treatment-review.json"
ROOT_FIELDS = {"contract_version", "title", "mainline_sha256", "decision_fissure_audit_sha256", "complete_story", "choices", "endings"}
...
def read_treatment(cache_root: Path) -> tuple[Path, dict[str, Any]]:
    ...
    for question, fissure in adopted.items():
        expected_actions = {
            str(action["action_text"]): str(action["immediate_consequence"])
            for action in fissure["actions"]
        }
        actual_actions = {
            str(option["option_text"]): str(option["immediate_consequence"])
